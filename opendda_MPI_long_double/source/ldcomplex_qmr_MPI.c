/* Written by James Mc Donald 2006
   ...
   lambda,theta,denom,temp,result,element: Local variables for iterative scheme
...
void ldcomplex_qmr_MPI(void){

   int vc,iteration=0;
   long int index0i,array0,array1,array2;
   ldcomplex tempdc={{0.0L,0.0L}},kappa={{-1.0L,0.0L}},kappa0,rho,rho0;
   ldcomplex epsilon,mu={{0.0L,0.0L}},tau,tau0,reduce;
   long double ratio,norm0,norm1,gamma0,gamma1,xi,xi0,lambda=1.0L;
   long double theta=-1.0L,denom,tempd,terminate;
...
/* r=M^{-1}r */
   ...
   /* omegatilde=vtilde=r */
   memcpy(omegatilde,r,(size_t)parallel.alloc_vector3*sizeof(ldcomplex));
   memcpy(vtilde,r,(size_t)parallel.alloc_vector3*sizeof(ldcomplex));
...
/*   gamma0=ldcomplex_2vectornorm(vtilde);
   xi=ldcomplex_2vectornorm(omegatilde); */
   gamma0=norm0;xi=norm0;
   reduce=zero;
   for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
      reduce=ldcomplex_add(reduce,ldcomplex_mul(omegatilde[index0i],vtilde[index0i]));
   }
   MPI_Allreduce(&reduce,&rho,1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
   dftmatvec_MPI(omegatilde);
   if(iterative.precond!=0){ /* Preconditioning enabled */
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* interaction_matrix*omegatilde=M^{-1}interaction_matrix*omegatilde */
   ...
   for(vc=0;vc<3;vc++){
      ...
      array1=vc*parallel.alloc_padded;
      for(index0i=0;index0i<parallel.alloc_vector;index0i++){
         reduce=ldcomplex_add(reduce,ldcomplex_mul(zero_padded_vector[array1+index0i],vtilde[array0+index0i]));
      }
   }
   MPI_Allreduce(&reduce,&epsilon,1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
...
/* 6. p=vtilde/gamma0-mu*p */
      ...
      tempd=1.0L/gamma0;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         p[index0i]=ldcomplex_sub(ldcomplex_scale(vtilde[index0i],tempd),ldcomplex_mul(mu,p[index0i]));
      }
...
/* 7. q=interaction_matrix^{T}omegatilde/xi-(gamma0*mu/xi)*q */
      ...
      tempdc=ldcomplex_scale(mu,gamma0/xi);
      tempd=1.0L/xi;
      dftmatvec_MPI(omegatilde);
      if(iterative.precond!=0){ /* Preconditioning enabled */
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* interaction_matrix*omegatilde=M^{-1}interaction_matrix*omegatilde */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            array2=array0+index0i;
            q[array2]=ldcomplex_sub(ldcomplex_scale(zero_padded_vector[array1+index0i],tempd),ldcomplex_mul(tempdc,q[array2]));
         }
      }

/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 8. vtilde=interaction_matrix*p-(tau/gamma0)vtilde */
      tempdc=ldcomplex_scale(tau,(1.0L/gamma0));
      dftmatvec_MPI(p);
      if(iterative.precond!=0){ /* Preconditioning enabled */
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* interaction_matrix*p=M^{-1}interaction_matrix*p */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            array2=array0+index0i;
            vtilde[array2]=ldcomplex_sub(zero_padded_vector[array1+index0i],ldcomplex_mul(tempdc,vtilde[array2]));
         }
      }

/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 9. omegatilde=q-(tau/xi)omegatilde */
      tempdc=ldcomplex_scale(tau,(1.0L/xi));
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         omegatilde[index0i]=ldcomplex_sub(q[index0i],ldcomplex_mul(tempdc,omegatilde[index0i]));
      }
...
/* 13. tau=(epsilon/rho)-gamma0*mu */
      ...
      tau0=tau;
      tau=ldcomplex_sub(ldcomplex_div(epsilon,rho),ldcomplex_scale(mu,gamma0));
...
/* 14. theta=(|tau0|^{2}(1-lambda))/(lambda*|tau0|^{2}+|gamma0|^{2}) */
      ...
      denom=(lambda*ldcomplex_abs2(tau0))+(gamma0*gamma0);
      theta=(ldcomplex_abs2(tau0)*(1.0L-lambda))/denom;
...
/* 15. kappa=(-gamma1*CONJ(tau0)*kappa0)/(lambda*|tau0|^{2}+|gamma0|^{2}) */
      ...
      tempdc=tau0;
      tempdc.dat[1]*=-1.0L;
      kappa=ldcomplex_scale(ldcomplex_scale(ldcomplex_mul(tempdc,kappa0),-gamma1),(1.0L/denom));

/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 16. lambda=(lambda*|tau0|^{2})/(lambda*|tau|^{2}+|gamma0|^{2}) */
      lambda=(lambda*ldcomplex_abs2(tau0))/denom;

/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 17. d=theta*d+kappa*p */
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         d[index0i]=ldcomplex_add(ldcomplex_scale(d[index0i],theta),ldcomplex_mul(kappa,p[index0i]));
      }
...
/* interaction_matrix*p=M^{-1}interaction_matrix*p */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            array2=array0+index0i;
            s[array2]=ldcomplex_add(ldcomplex_scale(s[array2],theta),ldcomplex_mul(kappa,zero_padded_vector[array1+index0i]));
         }
      }
...
/* 19. dipole_polarisation=dipole_polarisation+d */
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         dipole_polarisation[index0i]=ldcomplex_add(dipole_polarisation[index0i],d[index0i]);
      }

/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 20. r=r-s */
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         r[index0i]=ldcomplex_sub(r[index0i],s[index0i]);
      }
      iteration++;
   }
