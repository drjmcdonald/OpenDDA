...
void ldcomplex_rbicgstab_MPI(void){
   ...
   ldcomplex rhozero={{1.0L,0.0L}},rho,alpha={{0.0L,0.0L}},omega={{1.0L,0.0L}},beta,xi,reduce;
...
/* 6. rho=r[jj]^{T}rtilde */
         ...
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<parallel.alloc_vector;index0i++){
               reduce=ldcomplex_add(reduce,ldcomplex_mul(r[array1+index0i],rtilde[array0+index0i]));
            }
         }
...
/* 11. xi=u[jj+1]^{T}rtilde */
         ...
         for(vc=0;vc<3;vc++){
            ...
            array1=(jj+1)*parallel.alloc_vector3+vc*parallel.alloc_vector;
            for(index0i=0;index0i<parallel.alloc_vector;index0i++){
               reduce=ldcomplex_add(reduce,ldcomplex_mul(u[array1+index0i],rtilde[array0+index0i]));
            }
         }
         MPI_Allreduce(&reduce,&xi,1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
         /* Check for iterative.breakdown */
...
/* 17. sigma[0]=r[1]^{T}r[1], gamp[0]=(r[0]^{T}r[1])/(sigma[0]) */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            array2=array0+index0i;
            reduce=ldcomplex_add(reduce,ldcomplex_mul(r[array2],r[array2]));
         }
      }
      MPI_Allreduce(&reduce,&sigma[0],1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
      /* Check for iterative.breakdown */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            reduce=ldcomplex_add(reduce,ldcomplex_mul(r[array0+index0i],r[array1+index0i]));
         }
      }
      MPI_Allreduce(&reduce,&gamp[0],1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
...
/* 18. tau[ii-1][jj-1]=(r[jj]^{T}r[ii])/(sigma[ii-1]) */
            ...
            MPI_Allreduce(&reduce,&tau[ii-1][jj-1],1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
...
/* 19. sigma[jj-1]=(r[jj])^{T}r[jj], gamp[jj-1]=(r[0]^{T}r[jj])/(sigma[jj-1]) */
         ...
         MPI_Allreduce(&reduce,&sigma[jj-1],1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
         /* Check for iterative.breakdown */
         ...
         MPI_Allreduce(&reduce,&gamp[jj-1],1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
