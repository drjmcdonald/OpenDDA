...
/* Treat (proc,element)=(myrank,i) as a source and work out the destination */
         ...
         MPI_Isend(&zero_padded_vector[comp*parallel.alloc_padded+(i*target.K)],target.K,ldcomplex_type,j1%parallel.np,k1*before+j1,MPI_COMM_WORLD,&rqsend); /* Non-blocking send */
...
/* Treat (proc,element)=(myrank,i) as a destination and work out the source */
         ...
         MPI_Irecv(&zero_padded_vector[comp*parallel.alloc_padded+(i*target.K)],target.K,ldcomplex_type,k%parallel.np,k*before+j,MPI_COMM_WORLD,&rqrecv[cr]); /* Non-blocking receive */
         cr++;
      }
   }
...
/* Send the locally transposed data to the appropriate processors */
 ...
 /* Send the planes that are impeding the in-place receive.
         ...
         MPI_Isend(&zero_padded_vector[comp*parallel.alloc_padded+(i*index1)],index1,ldcomplex_type,i%parallel.np,index2+i,MPI_COMM_WORLD,&rqsend[cs]);
         cs++;i++;
      }
...
/* Buffering requirement is the bufsize to store the accumulated planes and
   ...
   bufsize+=parallel.np;
   ldcomplex_malloc(&buffer,(size_t)(bufsize*planesbefore*target.K)); /* Allocate the buffer */
