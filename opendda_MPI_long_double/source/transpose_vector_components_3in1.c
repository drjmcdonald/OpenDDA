/* Written by James Mc Donald 2006
   ...
   Transposes the 3 zero-padded vector components at the same time

   transpose_vector_component_3in1_0: Transposes zero_padded_vector[3][K*Jpp*P]
                                      line-by-line (lines in the x-direction)
   transpose_vector_component_3in1_1: Transposes zero_padded_vector[3][K*Jpp*P]
                                      plane-by-plane (includes a local transpose
                                      and only works if message size is below the
                                      MPI EAGER limit)
   transpose_vector_component_3in1_2: Transposes zero_padded_vector[3][K*Jpp*P]
                                      plane-by-plane (includes a local transpose) 
...
void transpose_vector_component_3in1_0(int before,int after,int limitbefore,int limitafter){
   ...
   int count3=3,blocklen3=target.K,stride3=parallel.alloc_padded;

/* Create vector to send and receive all 3 zero-padded vector components at the same time */
   MPI_Type_vector(count3,blocklen3,stride3,ldcomplex_type,&vector3);
   MPI_Type_commit(&vector3); /* Commit the vector */
...
/* Treat (proc,element)=(myrank,i) as a source and work out the destination */
         ...
         MPI_Isend(&zero_padded_vector[i*target.K],1,vector3,j1%parallel.np,k1*before+j1,MPI_COMM_WORLD,&rqsend); /* Non-blocking send */
...
/* Treat (proc,element)=(myrank,i) as a destination and work out the source */
         ...
         MPI_Irecv(&zero_padded_vector[i*target.K],1,vector3,k%parallel.np,k*before+j,MPI_COMM_WORLD,&rqrecv[cr]); /* Non-blocking receive */
         cr++;
      }
   }
   ...
   MPI_Type_free(&vector3); /* Free the vector */
}

void transpose_vector_component_3in1_1(int before,int after,int planesbefore,int planesafter,int recv){

   int i,j,limit,cs=0,cr=0,index0,index1,index2,vc;
   MPI_Request *rqrecv,*rqsend;
   MPI_Datatype vector_recv0,vector_recv1;
   MPI_Datatype vector_send3,vector_recv0_3,vector_recv1_3;
   int count_recv0=recv+1,count_recv1=recv,blocklen_recv=target.K,stride_recv=parallel.np*target.K;
   MPI_Aint hstride3=sizeof(ldcomplex)*parallel.alloc_padded;
   int blocklen3=planesbefore*target.K,stride3=parallel.alloc_padded;
...
/* Forward: If receiving from proc with `myrank' < Pp%np, each plane has `Znp+1' x-dirn lines
   ...
   MPI_Type_vector(count_recv0,blocklen_recv,stride_recv,ldcomplex_type,&vector_recv0);
/* Forward: If receiving from proc with `myrank' >= Pp%np, each plane has `Znp' x-dirn lines
   ...
   MPI_Type_vector(count_recv1,blocklen_recv,stride_recv,ldcomplex_type,&vector_recv1);
   MPI_Type_commit(&vector_recv0); /* Commit the vector */
   MPI_Type_commit(&vector_recv1); /* Commit the vector */

/* Create vector to receive all 3 zero-padded vector components at the same time
   ...
   Thus need to use MPI_Type_hvector where the stride is in bytes */
   MPI_Type_hvector(3,1,hstride3,vector_recv0,&vector_recv0_3);
   MPI_Type_hvector(3,1,hstride3,vector_recv1,&vector_recv1_3);
   MPI_Type_commit(&vector_recv0_3); /* Commit the vector */
   MPI_Type_commit(&vector_recv1_3); /* Commit the vector */

/* Create vector to send all 3 zero-padded vector components at the same time */
   MPI_Type_vector(3,blocklen3,stride3,ldcomplex_type,&vector_send3);
   MPI_Type_commit(&vector_send3); /* Commit the vector */
...
/* Each proc performs a local transpose on its portion of the 3 zero-padded vector components */
   for(vc=0;vc<3;vc++){
      local_transpose_vector_component(before,planesbefore,vc);
   }
...
/* Send the locally transposed data to the appropriate processors */
   ...
   while((i<before)||(index0<planesafter)){
      ...
      while((i*planesbefore)<((index0+1)*after)&&(i<before)){ /* Send the planes that are impeding the in-place receive.
         Note that this algorithm only works when the message size i.e. index1*sizeof(ldcomplex) < MPI_EAGER_LIMIT.
         ...
         MPI_Isend(&zero_padded_vector[i*index1],1,vector_send3,i%parallel.np,index2+i,MPI_COMM_WORLD,&rqsend[cs]);
         cs++;i++;
      }
      ...
      if(index0<planesafter){ /* Perform the receives */
         for(j=0;j<parallel.np;j++){
            if(j<limit){ /* Forward: If receiving from proc with `myrank' < Pp%np, each plane has `Znp+1' x-dirn lines
                           Reverse: If receiving from proc with `myrank' < Jp%np, each plane has `Ynp+1' x-dirn lines */
               MPI_Irecv(&zero_padded_vector[(index0*after+j)*target.K],1,vector_recv0_3,j,j*before+index0*parallel.np+parallel.myrank,MPI_COMM_WORLD,&rqrecv[cr]);
               cr++; /* Increment the number of non-blocking receives that are currently active */
                     ...
                     Reverse: If receiving from proc with `myrank' >= Jp%np, each plane has `Ynp' x-dirn lines */
               MPI_Irecv(&zero_padded_vector[(index0*after+j)*target.K],1,vector_recv1_3,j,j*before+index0*parallel.np+parallel.myrank,MPI_COMM_WORLD,&rqrecv[cr]);
               cr++; /* Increment the number of non-blocking receives that are currently active */
            }
         }
      }
...
/* Free the vector datatypes */
   ...
   MPI_Type_free(&vector_send3);
   MPI_Type_free(&vector_recv0_3);
   MPI_Type_free(&vector_recv1_3);
}

void transpose_vector_component_3in1_2(int before,int after,int planesbefore,int planesafter,int recv){

   int i,i0,j,k,limit,cr=0,cs,index0,index1,bufsize,buffind,vc;
   ldcomplex *buffer;
   MPI_Request *rqrecv,*rqsend;
   MPI_Datatype vector_recv0,vector_recv1,vector_recv0_3,vector_recv1_3;
   MPI_Aint hstride3=sizeof(ldcomplex)*parallel.alloc_padded;
   int count_recv0=recv+1,count_recv1=recv,blocklen_recv=target.K,stride_recv=parallel.np*target.K;
...
/* Create vector to receive all 3 zero-padded vector components at the same time
   Cannot use MPI_Type_vector for 2nd vector level structure as its stride
   is calculated in units of the base `oldtype' which is vector_recv0,vector_recv0
   and since actual required stride is `alloc_padded' cannot build it out of vector_recv0,vector_recv0.
   Thus need to use MPI_Type_hvector where the stride is in bytes */
   MPI_Type_hvector(3,1,hstride3,vector_recv0,&vector_recv0_3);
   MPI_Type_hvector(3,1,hstride3,vector_recv1,&vector_recv1_3);
   MPI_Type_commit(&vector_recv0_3); /* Commit the vector */
...
/* Buffering requirement is the bufsize to store the accumulated planes and `np' planes for the dynamic
   send and receive contribution */
   bufsize+=parallel.np;
   ldcomplex_malloc(&buffer,(size_t)(bufsize*planesbefore*target.K*3)); /* Allocate the buffer */
...
/* Send the locally transposed data to the appropriate processors */
   ...
   while(i<before||i0<before||index0<planesafter){
      while((i<((index0+1)*parallel.np)||(i*planesbefore)<((index0+1)*after))&&i<before){
         for(vc=0;vc<3;vc++){ /* Buffer the 3 component planes */
            for(k=0;k<index1;k++){
               buffer[((i%bufsize)*index1*3)+vc*index1+k]=zero_padded_vector[vc*parallel.alloc_padded+(i*index1+k)];
            }
         }
      ...
      for(j=0;j<parallel.np;j++){ /* Perform the sends */
         if(i0<before){
            MPI_Isend(&buffer[(i0%bufsize)*index1*3],index1*3,ldcomplex_type,i0%parallel.np,parallel.myrank*before+i0,MPI_COMM_WORLD,&rqsend[cs]);
