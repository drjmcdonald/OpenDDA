...
/* interaction_matrix*v=M^{-1}interaction_matrix*v */
      ...
      for(vc=0;vc<3;vc++){
         ...
         array1=vc*parallel.alloc_padded;
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            reduce=ldcomplex_add(reduce,ldcomplex_mul(v[array0+index0i],zero_padded_vector[array1+index0i]));
         }
      }
...
/* interaction_matrix*v is stored in zero_padded_vector */
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<parallel.alloc_vector;index0i++){
               array2=array0+index0i;
               vtilde[array2]=ldcomplex_sub(zero_padded_vector[array1+index0i],ldcomplex_add(ldcomplex_mul(alpha,v[array2]),ldcomplex_mul(vtilde[array2],beta)));
            }
         }
      }
...
/* 12. beta=sqrt[vtilde^{T}vtilde],omega=||vtilde||/|beta| */
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=ldcomplex_add(reduce,ldcomplex_mul(vtilde[index0i],vtilde[index0i]));
      }
      MPI_Allreduce(&reduce,&beta,1,ldcomplex_type,add_ldcomplex,MPI_COMM_WORLD);
      beta=ldcomplex_sqrt(beta);
      /* Check for iterative.breakdown */
...
/* 17. p=(v-eta*p-theta*pold)/zeta (tempdc0=1/zeta) */
         for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
            ...
            pold[index0i]=ldcomplex_mul(ldcomplex_sub(v[index0i],ldcomplex_add(ldcomplex_mul(eta,p[index0i]),ldcomplex_mul(theta,pold[index0i]))),tempdc0);
         }
