...
void cross_sections_MPI(void){
   ...
   long int index0i;
   long double k3,temp,temp1,result;
...
/* C_{ext} Extinction cross section
   ...
   MPI_Reduce(&temp,&result,1,MPI_LONG_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
   if(parallel.myrank==0){ /* Restrict to master */
      cross_section.extinction=(4.0L*ldpi*wavenumber*result)/E2.dat[0];
   }
...
/* C_{abs} Absorption cross section
   ...
   temp=0.0L;result=0.0L;
   for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
      temp1=ldcomplex_abs2(dipole_polarisation[index0i]);
      temp+=(-interaction_matrix_diagonal[index0i].dat[1]*temp1-(k3*temp1));
   }
   MPI_Reduce(&temp,&result,1,MPI_LONG_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
   if(parallel.myrank==0){ /* Restrict to master */
      cross_section.absorption=((4.0L*ldpi*wavenumber)/E2.dat[0])*result;
