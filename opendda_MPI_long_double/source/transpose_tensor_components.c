/* Written by James Mc Donald 2006
   ...
   Transposes the 6 independent tensor components separately
   ...
   comp: Tensor component to be transposed
...
/* Treat (proc,element)=(myrank,i) as a source and work out the destination */
         ...
         MPI_Isend(&interaction_matrix[comp*parallel.alloc_tensor+(i*target.Ka)],target.Ka,ldcomplex_type,j1%parallel.np,k1*before+j1,MPI_COMM_WORLD,&rqsend); /* Non-blocking send */
...
/* Treat (proc,element)=(myrank,i) as a destination and work out the source */
         ...
         MPI_Irecv(&interaction_matrix[comp*parallel.alloc_tensor+(i*target.Ka)],target.Ka,ldcomplex_type,k%parallel.np,k*before+j,MPI_COMM_WORLD,&rqrecv[cr]); /* Non-blocking receive */
         cr++;
      }
   }
...
/* Performs a local transpose on its portion of the independent tensor component specified by `comp' */
   local_transpose_tensor_component(before,planesbefore,comp);
...
/* Send the locally transposed data to the appropriate processors */
   ...
   while((i<before)||(index0<planesafter)){
      ...
      while((i*planesbefore)<((index0+1)*after)&&(i<before)){ /* Send the planes that are impeding the in-place receive.
         ...
         MPI_Isend(&interaction_matrix[comp*parallel.alloc_tensor+(i*index1)],index1,ldcomplex_type,i%parallel.np,index2+i,MPI_COMM_WORLD,&rqsend[cs]);
         cs++;i++;
      }
      ...
      if(index0<planesafter){ /* Perform the receives */
         for(j=0;j<parallel.np;j++){
            if(j<limit){ /* If receiving from proc with `myrank' < P%np, each plane has 'Znp0+1' x-dirn lines
                           If receiving from proc with `myrank' < Jp%np, each plane has 'Ynp+1' x-dirn lines */
               MPI_Irecv(&interaction_matrix[comp*parallel.alloc_tensor+((index0*after+j)*target.Ka)],1,vector_recv0,j,j*before+index0*parallel.np+parallel.myrank,MPI_COMM_WORLD,&rqrecv[cr]);
               cr++; /* Increment the number of non-blocking receives that are currently active */
                     ...
                     If receiving from proc with `myrank' >= Jp%np, each plane has 'Ynp' x-dirn lines */
               MPI_Irecv(&interaction_matrix[comp*parallel.alloc_tensor+((index0*after+j)*target.Ka)],1,vector_recv1,j,j*before+index0*parallel.np+parallel.myrank,MPI_COMM_WORLD,&rqrecv[cr]);
               cr++; /* Increment the number of non-blocking receives that are currently active */
            }
         }
      }
...
/* Buffering requirement is the bufsize to store the accumulated planes and
   ...
   bufsize+=parallel.np;
   ldcomplex_malloc(&buffer,(size_t)(bufsize*planesbefore*target.Ka)); /* Allocate the buffer */
   ...
   while(i<before||i0<before||index0<planesafter){
      while((i<((index0+1)*parallel.np)||(i*planesbefore)<((index0+1)*after))&&i<before){
         for(k=0;k<index1;k++){ /* Buffer the plane */
            buffer[(i%bufsize)*index1+k]=interaction_matrix[comp*parallel.alloc_tensor+(i*index1+k)];
         }
      ...
      for(j=0;j<parallel.np;j++){ /* Perform the sends */
         if(i0<before){
            MPI_Isend(&buffer[(i0%bufsize)*index1],index1,ldcomplex_type,i0%parallel.np,parallel.myrank*before+i0,MPI_COMM_WORLD,&rqsend[cs]);
