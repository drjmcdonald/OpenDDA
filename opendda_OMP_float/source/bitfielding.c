...
void bitfielding(void){
   ...
   ctrl=(int)((double)(target.N-1)/32.0+1.0);
   uint_calloc(&target.occupied,(size_t)ctrl);
   ...
   if(refractive_index.number_of_materials>1){ /* Composition array is only required
      ...
      ctrl=(int)(((double)(target.N-1)/(double)target.masks_per_integer)+1.0);

      /* Allocate the memory for the x, y & z composition data */
      for(vc=0;vc<3;vc++){
         uint_calloc(&target.composition[vc],(size_t)ctrl);
      }
   }
