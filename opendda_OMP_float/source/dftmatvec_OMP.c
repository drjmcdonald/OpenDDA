...
#pragma omp parallel for private(index0k,j,index0j,i,index0i,plan_tcxdft,thread) firstprivate(planflag) schedule(dynamic)
         for(k=0;k<target.P;k++){ /* Extract the J*P sections into xdft one at a time, make them circulant, DFT
            ...
            if(planflag==0){ /* Create FFTW plan if not done so already */
               ...
               {
                  plan_tcxdft=fftwf_plan_dft(1,&target.Kpp,(fftwf_complex *)(&xdft[thread][0]),(fftwf_complex *)(&xdft[thread][0]),-1,FFTW_MEASURE);
                  planflag=1;
               }
            }
            for(j=0;j<target.J;j++){
               ...
               for(i=0;i<target.K;i++){ 
                  ...
                  if(i>0){ /* Make circulant */
                     xdft[thread][target.Kpp-i]=fcomplex_scale(interaction_matrix[index0i],xsign[tc]); 
                  }
               }
...
#pragma omp parallel for private(index0k,i,index0i,j,index0j,plan_tcydft,thread) firstprivate(planflag) schedule(dynamic)
         for(k=0;k<target.P;k++){ /* Extract the Ka*P sections into ydft one at a time, make them circulant, DFT
            ...
            if(planflag==0){ /* Create FFTW plan if not done so already */
               #pragma omp critical /* FFTW plan creation is NOT thread safe so restrict to one thread at a time */
               {
                  plan_tcydft=fftwf_plan_dft(1,&target.Jpp,(fftwf_complex *)(&ydft[thread][0]),(fftwf_complex *)(&ydft[thread][0]),-1,FFTW_MEASURE);
                  planflag=1;
               }
            }
            for(i=0;i<target.Ka;i++){
               ...
               for(j=0;j<target.J;j++){
                  ...
                  if(j>0){ /* Make circulant */
                     ydft[thread][target.Jpp-j]=fcomplex_scale(interaction_matrix[index0j],ysign[tc]); 
                  }
               }
...
#pragma omp parallel for private(index0j,i,index0i,k,index0k,plan_tczdft,thread) firstprivate(planflag) schedule(dynamic)
         for(j=0;j<target.Ja;j++){ /* Extract the Ja*P sections into zdft one at a time, make them circulant, DFT
            ...
            if(planflag==0){ /* Create FFTW plan if not done so already */
               #pragma omp critical /* FFTW plan creation is NOT thread safe so restrict to one thread at a time */
               {
                  plan_tczdft=fftwf_plan_dft(1,&target.Ppp,(fftwf_complex *)(&zdft[thread][0]),(fftwf_complex *)(&zdft[thread][0]),-1,FFTW_MEASURE);
                  planflag=1;
               }
             }
            for(i=0;i<target.Ka;i++){
               ...
               for(k=0;k<target.P;k++){
                  ...
                  if(k>0){ /* Make circulant */
                     zdft[thread][target.Ppp-k]=fcomplex_scale(interaction_matrix[index0k],zsign[tc]);
                  }
               }
...
#pragma omp parallel for private(jj,jsign,index0j,index1j,thread,plan_vczdft,plan_vczdfti,vc,array1,array2,k,kk,ksign,index0k,index1k,i,ii,isign,index0i,index1i,m0,m1,m2,m3,m4,m5,v0,v1,v2) firstprivate(planflag) schedule(dynamic)
   for(j=0;j<target.Jpp;j++){
      ...
      if(planflag==0){ /* Create FFTW plans if not done so already */
         #pragma omp critical /* FFTW plan creation is NOT thread safe so restrict to one thread at a time */
         {
            plan_vcxdft=fftwf_plan_many_dft(1,&target.Kpp,target.P,(fftwf_complex *)(&xzscratch[thread][0]),NULL,1,target.Kpp,(fftwf_complex *)(&xzscratch[thread][0]),NULL,1,target.Kpp,-1,FFTW_MEASURE);
            plan_vczdft=fftwf_plan_many_dft(1,&target.Ppp,target.Kpp,(fftwf_complex *)(&xzscratch[thread][0]),NULL,target.Kpp,1,(fftwf_complex *)(&xzscratch[thread][0]),NULL,target.Kpp,1,-1,FFTW_MEASURE);
      plan_vcxdfti=fftwf_plan_many_dft(1,&target.Kpp,target.P,(fftwf_complex *)(&xzscratch[thread][0]),NULL,1,target.Kpp,(fftwf_complex *)(&xzscratch[thread][0]),NULL,1,target.Kpp,+1,FFTW_MEASURE);
            plan_vczdfti=fftwf_plan_many_dft(1,&target.Ppp,target.Kpp,(fftwf_complex *)(&xzscratch[thread][0]),NULL,target.Kpp,1,(fftwf_complex *)(&xzscratch[thread][0]),NULL,target.Kpp,1,+1,FFTW_MEASURE);
            planflag=1;
         }
       }
      ...
      for(vc=0;vc<3;vc++){ /* xDFT */
         fftwf_execute_dft(plan_vcxdft,(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]),(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]));
      }
      for(vc=0;vc<3;vc++){ /* zDFT */
         fftwf_execute_dft(plan_vczdft,(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]),(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]));
      }
      ...
      for(vc=0;vc<3;vc++){ /* ziDFT */
         fftwf_execute_dft(plan_vczdfti,(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]),(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]));
      }
      for(vc=0;vc<3;vc++){ /* xiDFT */
         fftwf_execute_dft(plan_vcxdfti,(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]),(fftwf_complex *)(&xzscratch[thread][vc*target.Qpp]));
      }
