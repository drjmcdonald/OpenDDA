...
/* ->->->->->->->->->->->-> Incident wave ->->->->->->->->->->->-> */
   ...
   fprintf(output.incident_wave,"\n%d. Incident polarisation e0=[%.*g%+.*gi]x+[%.*g%+.*gi]y\n",count++,FLTP,incident_LF.polarisation[0].dat[0],FLTP,incident_LF.polarisation[0].dat[1],FLTP,incident_LF.polarisation[1].dat[0],FLTP,incident_LF.polarisation[1].dat[1]);
   if(polarisations==1){
      fprintf(output.incident_wave,"\n%d. Calculations for the orthonormal polarisation state e1 were NOT included\n",count++);
   }
   else{
      fprintf(output.incident_wave,"\n%d. Calculations for the orthonormal polarisation state\n   e1=[%.*g%+.*gi]x+[%.*g%+.*gi]y were included\n",count++,FLTP,incident_LF.orthonormal[0].dat[0],FLTP,incident_LF.orthonormal[0].dat[1],FLTP,incident_LF.orthonormal[1].dat[0],FLTP,incident_LF.orthonormal[1].dat[1]);
   }
   ...
   if(wavelength.flag==0){ /* Use minimum,increment,maximum from control.input */
      fprintf(output.incident_wave,"\n%d. Incident wavelength [micrometers]:\n\n   Used minimum,increment,maximum from control.input\n   minimum=%.*g\n   increment=%.*g\n   maximum=%.*g\n\n",count++,FLTP,wavelength.limits[0],FLTP,wavelength.limits[1],FLTP,wavelength.limits[2]); /* Print description */
...
/* ->->->->->->->->->->->-> Effective radius ->->->->->->->->->->->-> */
   ...
   if(radius.flag==0){ /* Use minimum,increment,maximum from control.input */
      fprintf(output.eff_radius,"\n%d. Effective radius [micrometers]:\n\n   Used minimum,increment,maximum from control.input\n   minimum=%.*g\n   increment=%.*g\n   maximum=%.*g\n\n",count++,FLTP,radius.limits[0],FLTP,radius.limits[1],FLTP,radius.limits[2]); /* Print description */
      ...
      fprintf(output.eff_radius,"   0. \t%.*g\n",FLTP,radius.value); /* Print values */
...
/* ->->->->->->->->->->->-> Target orientations ->->->->->->->->->->->-> */
   ...
   if(euler_phi.flag==0){ /* Used minimum,increment,maximum from control.input */
      fprintf(output.target_orien,"\n%d. 1st Euler angle - phi - Azimuthal angle:\n\n   Used minimum,increment,maximum from control.input\n   minimum=%.*g\n   increment=%.*g\n   maximum=%.*g\n\n",count++,FLTP,euler_phi.limits[0],FLTP,euler_phi.limits[1],FLTP,euler_phi.limits[2]); /* Print description */
   ...
   if(euler_theta.flag==0){ /* Used minimum,increment,maximum from control.input */
      ...
      fprintf(output.target_orien,"   0. \t%+.*g\t\t%.*g\n",FLTP,euler_theta.value,FLTP,radians_to_degrees(acosf(euler_theta.value))); /* Print values */
      for(i=1;i<euler_theta.list_length;i++){
         euler_theta.value+=euler_theta.limits[1];
         fprintf(output.target_orien,"   %d. \t%+.*g\t\t%.*g\n",i,FLTP,euler_theta.value,FLTP,radians_to_degrees(acosf(euler_theta.value)));
      }
   }
      ...
      for(i=0;i<euler_theta.list_length;i++){ /* Print values */
         fprintf(output.target_orien,"   %d. \t%+.*g\t\t%.*g\n",i,FLTP,euler_theta.list[i],FLTP,radians_to_degrees(acosf(euler_theta.list[i])));
      }
   }   
   /* Euler psi */
   if(euler_psi.flag==0){ /* Used minimum,increment,maximum from control.input */
      fprintf(output.target_orien,"\n%d. 3rd Euler angle - psi - Azimuthal angle:\n\n   Used minimum,increment,maximum from control.input\n   minimum=%.*g\n   increment=%.*g\n   maximum=%.*g\n\n",count++,FLTP,euler_psi.limits[0],FLTP,euler_psi.limits[1],FLTP,euler_psi.limits[2]); /* Print description */
...
/* ->->->->->->->->->->->-> Program execution information ->->->->->->->->->->->-> */
   if(timing.enabled){
      ...
      print_section_title(output.time,"OpenMP information");
      fprintf(output.time,"\n  The architecture has %d threads available, of which, %d were used\n",threads_available,openmp_threads);      
      print_section_title(output.time,"Runtime, iterative kernel & DFT engine timing information");
      fprintf(output.time,"\n  Note: The number in square brackets after each time specifies\n        the number of executions in the average\n"); 
   }
}
