...
void memory_free_OMP(void){
   ...
   fcomplex_free(&dipole_polarisation); /* Dipole polarisations */
   fcomplex_free(&incident_E_field); /* Incident electric field */
   fcomplex_fftwf_free(&zero_padded_vector); /* Zero-padded vector for DFT matrix-vector multiplication */
   fcomplex_free(&interaction_matrix); /* 1st column of the interaction matrix with zero-diagonal */
   fcomplex_free(&interaction_matrix_diagonal); /* Diagonal of the interaction matrix */
   ...
   for(i=0;i<openmp_threads;i++){
      fcomplex_fftwf_free(&xdft[i]);
      fcomplex_fftwf_free(&ydft[i]);
      fcomplex_fftwf_free(&zdft[i]);
   }
   fcomplex_fftwf_free_ptr(&xdft);  /* xdft[openmp_threads][Kpp] */
   fcomplex_fftwf_free_ptr(&ydft);  /* ydft[openmp_threads][Jpp] */
   fcomplex_fftwf_free_ptr(&zdft);  /* zdft[openmp_threads][Ppp] */

   /* Scratch arrays for the DFT, FD multiplication & iDFT of the 3 zero-padded vector components [1 for each thread] */
   for(i=0;i<openmp_threads;i++){
      fcomplex_fftwf_free(&xzscratch[i]);
   }
   fcomplex_fftwf_free_ptr(&xzscratch);  /* xzscratch[openmp_threads][3*Qpp] */

   if(iterative.precond==1){ /* Point-Jacobi Preconditioning */
      fcomplex_free(&point_jacobi);
   }

   /* If data list was read in from file then memory was allocated to store it */
   if(wavelength.flag==1){ /* List was read from file */
      float_free(&wavelength.list); /* Wavelengths */
   }
   if(radius.flag==1){ /* List was read from file */
      float_free(&radius.list); /* Effective radii */
   }
   if(euler_phi.flag==1){ /* List was read from file */
      float_free(&euler_phi.list); /* Euler phi */
   }
   if(euler_theta.flag==1){ /* List was read from file */
      float_free(&euler_theta.list); /* Euler theta */
   }
   if(euler_psi.flag==1){ /* List was read from file */
      float_free(&euler_psi.list); /* Euler psi */
   }
   if(phi.flag==1){ /* List was read from file */
      float_free(&phi.list); /* Scattering angle phi */
   }
   if(theta.flag==1){ /* List was read from file */
      float_free(&theta.list); /* Scattering angle theta */
   }
   ...
   for(i=0;i<refractive_index.number_of_materials;i++){
      if(refractive_index.flag[i]==1){ /* Data was read from file for interpolation */
         float_free(&refractive_index.wavelengths[i]);
         fcomplex_free(&refractive_index.refractive_indices[i]);
      }
   }
   float_free_ptr(&refractive_index.wavelengths); /* Free pointers */
   fcomplex_free_ptr(&refractive_index.refractive_indices); /* Free pointers */

   fcomplex_free(&refractive_index.value); /* Current refractive index values */
   ...
   if(strcmp(iterative.scheme,"bicg")==0){
      fcomplex_free(&r);
      fcomplex_free(&rtilde);
      fcomplex_free(&p);
      fcomplex_free(&ptilde);
      fcomplex_free(&q);
   }
      ...
      fcomplex_free(&zd);
      fcomplex_free(&zg);
      fcomplex_free(&zomega);
      if(iterative.precond!=0){ /* Preconditioning enabled */
         fcomplex_free(&gtilde);
         fcomplex_free(&utilde);
      }
      fcomplex_free(&c);
   }
      ...
      fcomplex_free(&q);
      fcomplex_free(&s);
      fcomplex_free(&vtilde);
      fcomplex_free(&omegatilde);
   }
