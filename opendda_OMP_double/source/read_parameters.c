...
e1=[0,0,1] CROSS conj(e0) */
            ...
            for(a=0;a<3;a++){ /* a=0 x,a=1 y,a=2 z */
               for(b=0;b<2;b++){ /* b=0 real component,b=1 imaginary component */
                  ...
                  incident_LF.polarisation[a].dat[b]=atof(compare); /* string to double conversion */
            ...
            if(dcomplex_abs(incident_LF.polarisation[2])>DBL_EPSILON){
            ...
            normalise=dcomplex_add(dcomplex_mul(incident_LF.polarisation[0],dcomplex_conj(incident_LF.polarisation[0])),
                                    dcomplex_mul(incident_LF.polarisation[1],dcomplex_conj(incident_LF.polarisation[1])));
            /* Check normalisation value */
            if(normalise.dat[0]<DBL_EPSILON){
            ...
            incident_LF.polarisation[0]=dcomplex_scale(incident_LF.polarisation[0],normalise.dat[0]);
            incident_LF.polarisation[1]=dcomplex_scale(incident_LF.polarisation[1],normalise.dat[0]);
...
refractive indices, m=n+ki, for the target, 0 to (number_of_materials-1) */
            ...
            double_malloc_ptr(&refractive_index.wavelengths,(size_t)refractive_index.number_of_materials);
            dcomplex_malloc_ptr(&refractive_index.refractive_indices,(size_t)refractive_index.number_of_materials);
               ...
               if(strcmp(compare,temp)==0){
                  ...
                  if((data=fopen(temp,"r"))!=NULL){ /* Check if the appropriate input file exists */
                     ...
                     for(a=0;a<2;a++){ /* a=0 real component,a=1 imaginary component */
                        ...
                        refractive_index.value[j].dat[a]=atof(compare); /* string to double conversion */
                     }
                  }
...
If "target_orientation_euler_theta.input" exists read in data else use minimum,increment,maximum in "control.input" */
            ...
            if((data=fopen("INPUT_target_orientation_euler_theta.input","r"))!=NULL){
               ...
               for(m=0;m<3;m++){ /* Read in minimum,increment,maximum */
                  ...
                  euler_theta.limits[m]=atof(compare); /* string to double conversion */
                  while(((ispunct(string[i+1]))!=0)&&(string[i+1]!='-')){i++;} /* Skip all punctuation (except '-') */
               }
               /* Check theta values */
               if(euler_theta.limits[1]<0.0){
...
If "scattering_direction_phi.input" exists read in data else use minimum,increment,maximum in "control.input" */
            ...
            if((data=fopen("INPUT_scattering_direction_phi.input","r"))!=NULL){
               ...
               for(m=0;m<3;m++){ /* Use minimum,increment,maximum in "control.input" */
                  ...
                  phi.limits[m]=atof(compare); /* string to double conversion */
                  while(((ispunct(string[i+1]))!=0)&&(string[i+1]!='-')){i++;} /* Skip all punctuation (except '-') */
               }
               /* Check phi values */
               if(phi.limits[0]<0.0){
...
Iterative scheme - Convergence tolerance */
            ...
            iterative.tolerance=atof(compare); /* string to double conversion */
            /* Check tolerance value */
            if(iterative.tolerance<0.0){
...
Iterative scheme - Breakdown tolerance */
            ...
            iterative.breakdown=atof(compare); /* string to double conversion */
            /* Check breakdown value */
            if(iterative.breakdown<0.0){
...
/* Reads in the double data from the file pointed to by *file_ptr (ignoring comments) to the array
   ...
   double_malloc(&(*structure_ptr).list,(size_t)count); /* Allocate memory to store the data */
   ...
   *file_ptr=fopen(file_string,"r"); /* Reopen the data file for reading */
   ...
   while((fgets(string,sizeof(string),*file_ptr))!=NULL){
      if(string[0]=='#'){continue;} /* Ignore comments */
         ...
         ((*structure_ptr).list)[count]=atof(string); /* string to double conversion */
         if((strcmp(data_string,"Wavelength")==0)||(strcmp(data_string,"Effective radius")==0)){
            /* Wavelength and effective radius values must be > 0 */
            if(((*structure_ptr).list)[count]<DBL_EPSILON){ /* Input data MUST be > 0 */
               reset_string(string);
               ...
               sprintf(string2,"([%d:%.*g] <= 0) %s data MUST be > 0",count,DBL_DIG,((*structure_ptr).list)[count],data_string);
            ...
            if(((*structure_ptr).list)[count]<0.0){ /* Input data MUST be positive */
               reset_string(string);
               reset_string(string1);
               reset_string(string2);
               sprintf(string,"Please check the file %s",file_string);
         ...
         if(count==0){ /* Set initial value of previous */
            ...
            if((previous>((*structure_ptr).list)[count])||((((*structure_ptr).list)[count]-previous)<DBL_EPSILON)){
               ...
               sprintf(string,"Please check the file %s",file_string);
               sprintf(string1,"%s input error",data_string);
               sprintf(string2,"([%d:%.*g] >= [%d:%.*g]) %s data MUST be monotonically increasing",count-1,DBL_DIG,((*structure_ptr).list)[count-1],count,DBL_DIG,((*structure_ptr).list)[count],data_string);
               print_error(string1,string2,string);
            }
...
/* Reads in the wavelength and complex refractive index data from the file
   ...
   int a,i,k,count=0;
   double previous; /* Check if wavelength data is monotonically increasing */
   ...
   double_malloc(&(*structure_ptr).wavelengths[material],(size_t)count); /* Allocate memory to store the wavelength data */
   dcomplex_malloc(&(*structure_ptr).refractive_indices[material],(size_t)count); /* Allocate memory to store the complex refractive index data */
   ...
   *file_ptr=fopen(file_string,"r"); /* Reopen the data file for reading */
   ...
   while((fgets(string,sizeof(string),*file_ptr))!=NULL){
      if(string[0]=='#'){continue;} /* Ignore comments */
         ...
         ((*structure_ptr).wavelengths[material])[count]=atof(compare); /* string to double conversion */
         if(((*structure_ptr).wavelengths[material])[count]<DBL_EPSILON){ /* Wavelength data MUST be positive */
            reset_string(string);
            reset_string(compare);
            sprintf(compare,"([%d:%.*g] <= 0) The wavelength data MUST be > 0",count,DBL_DIG,((*structure_ptr).wavelengths[material])[count]);
         ...
         if(count==0){ /* Set initial value of previous */
            ...
            if((previous>((*structure_ptr).wavelengths[material])[count])||((((*structure_ptr).wavelengths[material])[count]-previous)<DBL_EPSILON)){
               /* If data is not monotonically increasing */
               reset_string(string);
               reset_string(compare);
               sprintf(string,"Please check the file INPUT_ref_index_vs_wavelength_material_%d.input",material);
               sprintf(compare,"([%d:%.*g] >= [%d:%.*g]) Wavelength data MUST be monotonically increasing",count-1,DBL_DIG,((*structure_ptr).wavelengths[material])[count-1],count,DBL_DIG,((*structure_ptr).wavelengths[material])[count]);
               print_error("Complex refractive index vs wavelength input error",compare,string);
            }
         ...
         for(a=0;a<2;a++){ /* a=0 real component,a=1 imaginary component */
            ...
            while(((ispunct(string[i+1])!=0)&&(string[i+1]!='-'))||(string[i+1]=='i')||(string[i+1]=='I')){i++;} /* Skip 'i', 'I' and all punctuation (except '-') */
            ((*structure_ptr).refractive_indices[material])[count].dat[a]=atof(compare); /* string to double conversion */
         }
         count++; /* Next entry */
      }
      reset_string(string);
