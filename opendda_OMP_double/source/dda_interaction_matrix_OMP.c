...
void dda_interaction_matrix_1st_column_zero_diagonal(void){
   ...
   double r,r2,r3,k2,x0[3],index1,index2;
   dcomplex frac0,frac1,ikr,eikr;

   k2=wavenumber*wavenumber; /* k^{2} */

   /* Only need to compute if 1st time or wavenumber has changed */
   if((wavenumber_previous<0.0)||fabs(wavenumber_previous-wavenumber)>DBL_EPSILON){
...
#pragma omp parallel for private(index0k,x0,j,index0j,i,index0i,r2,r,r3,ikr,eikr,frac0,frac1,tc,a,b,array0,index1,index2) schedule(dynamic)
      for(k=0;k<target.P;k++){
         index0k=k*target.Ma;
         x0[2]=((double)k)*target.dipole_spacing; /* z */
         for(j=0;j<target.J;j++){
            index0j=index0k+j*target.Ka;
            x0[1]=((double)j)*target.dipole_spacing; /* y */
            for(i=0;i<target.K;i++){
               if(i==0&&j==0&&k==0){continue;}
               index0i=index0j+i;
               x0[0]=((double)i)*target.dipole_spacing; /* x */
               r2=(x0[0]*x0[0]+x0[1]*x0[1]+x0[2]*x0[2]); /* r^2 */
               r=sqrt(r2); /* r */
               r3=r2*r; /* r^3 */
               ikr.dat[0]=0.0;ikr.dat[1]=wavenumber*r; /* ikr=k*r*i */
               ...
               eikr.dat[0]=cos(ikr.dat[1]);eikr.dat[1]=sin(ikr.dat[1]); /* e^{ikr} */
               frac0.dat[0]=eikr.dat[0]*(1.0/r3);frac0.dat[1]=eikr.dat[1]*(1.0/r3); /* frac0=eikr/r3 */
               frac1.dat[0]=(-1.0/r2);frac1.dat[1]=(ikr.dat[1])*(1.0/r2); /* frac1=(ikr-1)/r2 */
               tc=0;
               for(a=0;a<3;a++){
                  for(b=a;b<3;b++){
                     array0=tc*target.Na+index0i;
                     if(a==b){ /* Diagonal entries */
                        index1=3.0*x0[a]*x0[b]-r2; /* (3r*r-r^{2}) */
                        ...
                        index1=3.0*x0[a]*x0[b]; /* For the off-diagonal entries (r^{2}-3r*r) becomes (3r*r) */
