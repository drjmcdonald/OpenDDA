...
void incident_electric_field_S(void){
   ...
   long double wave_vector_TF[3],kr[3];
   ...
   for(k=1;k<=target.P;k++){
      ...
      kr[2]=wave_vector_TF[2]*((long double)k); /* kz*rz */
      for(j=1;j<=target.J;j++){
         index0j=index0k+(j-1)*target.K;
         kr[1]=kr[2]+wave_vector_TF[1]*((long double)j); /* ky*ry+kz*rz */
         for(i=1;i<=target.K;i++){
            index0i=index0j+(i-1);
            kr[0]=kr[1]+wave_vector_TF[0]*((long double)i); /* k.r=kx*rx+ky*ry+kz*rz */
            ikr.dat[0]=0.0L;ikr.dat[1]=kr[0]; /* ikr */
            ...
            if(target.occupied[(int)((double)index0i/32.0)]&(1<<(index0i%32))){ /* Lattice site is occupied */
               for(vc=0;vc<3;vc++){
                  incident_E_field[element+vc*target.Nd]=ldcomplex_mul(eikr,current_polarisation_TF[vc]);
               }
               element++;
            }
         }
