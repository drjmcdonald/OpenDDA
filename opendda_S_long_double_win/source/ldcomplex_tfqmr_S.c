...
void ldcomplex_tfqmr_S(void){

   int m,vc,iteration=0,exitflag=0;
   long int index0i,array0,array1,array2;
   ldcomplex eta={{0.0L,0.0L}},eta0,rho,rhok,sigma,alpha,beta,temp,*ptr;
...
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 2. omega=yb=r */
   memcpy(omega,r,(size_t)target.Nd3*sizeof(ldcomplex));
   memcpy(yb,r,(size_t)target.Nd3*sizeof(ldcomplex));
...
/* 8. rhok=rtilde^{T}r */
   rhok=zero;
   for(index0i=0;index0i<target.Nd3;index0i++){
      rhok=ldcomplex_add(rhok,ldcomplex_mul(rtilde[index0i],r[index0i]));
   }
...
/* 9. sigma=rtilde^{T}v */
      sigma=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         sigma=ldcomplex_add(sigma,ldcomplex_mul(rtilde[index0i],v[index0i]));
      }
...
/* 17. d=ya+(theta0^{2}*eta0/alpha)*d */
         temp=ldcomplex_scale(ldcomplex_div(eta0,alpha),theta0*theta0);
         for(index0i=0;index0i<target.Nd3;index0i++){
            d[index0i]=ldcomplex_add(ya[index0i],ldcomplex_mul(temp,d[index0i]));
         }
...
/* 21. rhok=rtilde^{T}omega */
      ...
      rhok=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         rhok=ldcomplex_add(rhok,ldcomplex_mul(rtilde[index0i],omega[index0i]));
      }
      /* Check for iterative.breakdown */
