...
void ldcomplex_rbicgstab_S(void){

   int i,ii,jj,vc,iteration=0;
   long int index0i,array0,array1,array2;
   ldcomplex rhozero={{1.0L,0.0L}},rho,alpha={{0.0L,0.0L}},omega={{1.0L,0.0L}},beta,xi;
...
/* 6. rho=r[jj]^{T}rtilde */
         ...
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<target.Nd;index0i++){
               rho=ldcomplex_add(rho,ldcomplex_mul(r[array1+index0i],rtilde[array0+index0i]));
            }
         }
...
/* 11. xi=u[jj+1]^{T}rtilde */
         ...
         for(vc=0;vc<3;vc++){
            ...
            array1=(jj+1)*target.Nd3+vc*target.Nd;
            for(index0i=0;index0i<target.Nd;index0i++){
               xi=ldcomplex_add(xi,ldcomplex_mul(u[array1+index0i],rtilde[array0+index0i]));
            }
         }
         /* Check for iterative.breakdown */
...
/* 17. sigma[0]=r[1]^{T}r[1], gamp[0]=(r[0]^{T}r[1])/(sigma[0]) */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<target.Nd;index0i++){
            ...
            sigma[0]=ldcomplex_add(sigma[0],ldcomplex_mul(r[array2],r[array2]));
         }
      }
      /* Check for iterative.breakdown */
      ...
      for(vc=0;vc<3;vc++){
         ...
         for(index0i=0;index0i<target.Nd;index0i++){
            gamp[0]=ldcomplex_add(gamp[0],ldcomplex_mul(r[array0+index0i],r[array1+index0i]));
         }
      }
...
/* 18. tau[ii-1][jj-1]=(r[jj]^{T}r[ii])/(sigma[ii-1]) */
            ...
            for(vc=0;vc<3;vc++){
               ...
               array1=ii*target.Nd3+vc*target.Nd;
               for(index0i=0;index0i<target.Nd;index0i++){
                  tau[ii-1][jj-1]=ldcomplex_add(tau[ii-1][jj-1],ldcomplex_mul(r[array0+index0i],r[array1+index0i]));
               }
            }
...
/* 19. sigma[jj-1]=(r[jj])^{T}r[jj], gamp[jj-1]=(r[0]^{T}r[jj])/(sigma[jj-1]) */
         ...
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<target.Nd;index0i++){
               array2=index0i+array0;
               sigma[jj-1]=ldcomplex_add(sigma[jj-1],ldcomplex_mul(r[array2],r[array2]));
            }
         }
         /* Check for iterative.breakdown */
         ...
         for(vc=0;vc<3;vc++){
            ...
            array1=jj*target.Nd3+vc*target.Nd;
            for(index0i=0;index0i<target.Nd;index0i++){
               gamp[jj-1]=ldcomplex_add(gamp[jj-1],ldcomplex_mul(r[array0+index0i],r[array1+index0i]));
