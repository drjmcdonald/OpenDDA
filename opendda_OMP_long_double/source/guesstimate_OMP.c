...
int main(void){
   ...
   int i,j,k,error,shape,file,threads;
   ...
   double sizedc=(double)sizeof(ldcomplex);
   FILE *data;
...
/* ------------------------------------------------------------------------- */
/* threads */
   printf("Please enter the number of OpenMP threads\n");
   threads=-1;
   while(threads<1){
      error=0; /* Initialise error */
      reset_string(in); /* Reset string */
      ...
      if(error==1){ /* If any character is not a digit then print an error */
         threads=-1;printf("\nMemory 'guesstimation' error\nArgument for the number of OpenMP threads contained an invalid character!\n\nPlease re-enter\n");
      }
      else{
         threads=atoi(in); /* Convert to an integer value */
         if(threads<1){ /* Check for any an invalid value */
            threads=-1;printf("\nMemory 'guesstimation' error\nThe number of OpenMP threads MUST be an integer > 0\n\nPlease re-enter\n");
         }
      }
   }
   ...
   /* DFT scratch arrays */
   base_memory+=((double)target.Kpp*(double)threads); /* xdft[Kpp] */
   base_memory+=((double)target.Jpp*(double)threads); /* ydft[Jpp] */
   base_memory+=((double)target.Ppp*(double)threads); /* zdft[Ppp] */

   /* DFT and FD multiplication scratch arrays */
   base_memory+=((double)target.Qpp*3.0*(double)threads); /* xzscratch[3*Qpp] */
