...
void read_parameters(void){
   ...
   incident_LF.n_inc[0]=0.0L;
   incident_LF.n_inc[1]=0.0L;   
   incident_LF.n_inc[2]=1.0L;
...
e1=[0,0,1] CROSS conj(e0) */
            ...
            for(a=0;a<3;a++){ /* a=0 x,a=1 y,a=2 z */
               for(b=0;b<2;b++){ /* b=0 real component,b=1 imaginary component */
                  ...
                  incident_LF.polarisation[a].dat[b]=(long double)atof(compare); /* string to long double conversion */
            ...
            if(ldcomplex_abs(incident_LF.polarisation[2])>LDBL_EPSILON){
            ...
            normalise=ldcomplex_add(ldcomplex_mul(incident_LF.polarisation[0],ldcomplex_conj(incident_LF.polarisation[0])),
                                    ldcomplex_mul(incident_LF.polarisation[1],ldcomplex_conj(incident_LF.polarisation[1])));
            /* Check normalisation value */
            if(normalise.dat[0]<LDBL_EPSILON){
            ...
            normalise.dat[0]=1.0L/sqrtl(normalise.dat[0]);
            incident_LF.polarisation[0]=ldcomplex_scale(incident_LF.polarisation[0],normalise.dat[0]);
            incident_LF.polarisation[1]=ldcomplex_scale(incident_LF.polarisation[1],normalise.dat[0]);
            ...
            if(strcmp(compare,"yes")==0){ /* Include the orthonormal polarisation state */
               ...
               incident_LF.orthonormal[0]=ldcomplex_mul(ldcomplex_conj(incident_LF.polarisation[1]),minusone); /* x-component */
               incident_LF.orthonormal[1]=ldcomplex_conj(incident_LF.polarisation[0]); /* y-component */
            }
...
refractive indices, m=n+ki, for the target, 0 to (number_of_materials-1) */
            ...
            long_double_malloc_ptr(&refractive_index.wavelengths,(size_t)refractive_index.number_of_materials);
            ldcomplex_malloc_ptr(&refractive_index.refractive_indices,(size_t)refractive_index.number_of_materials);
               ...
               if(strcmp(compare,temp)==0){
                  ...
                  if((data=fopen(temp,"r"))!=NULL){ /* Check if the appropriate input file exists */
                     ...
                     for(a=0;a<2;a++){ /* a=0 real component,a=1 imaginary component */
                        ...
                        refractive_index.value[j].dat[a]=(long double)atof(compare); /* string to long double conversion */
                     }
                  }
...
If "target_orientation_euler_theta.input" exists read in data else use minimum,increment,maximum in "control.input" */
            ...
            if((data=fopen("INPUT_target_orientation_euler_theta.input","r"))!=NULL){
               ...
               for(m=0;m<3;m++){ /* Read in minimum,increment,maximum */
                  ...
                  euler_theta.limits[m]=(long double)atof(compare); /* string to long double conversion */
                  while(((ispunct(string[i+1]))!=0)&&(string[i+1]!='-')){i++;} /* Skip all punctuation (except '-') */
               }
               /* Check theta values */
               if(euler_theta.limits[1]<0.0L){
...
Iterative scheme - Convergence tolerance */
            ...
            iterative.tolerance=(long double)atof(compare); /* string to long double conversion */
            /* Check tolerance value */
            if(iterative.tolerance<0.0L){
...
Iterative scheme - Breakdown tolerance */
            ...
            iterative.breakdown=(long double)atof(compare); /* string to long double conversion */
            /* Check breakdown value */
            if(iterative.breakdown<0.0L){
...
/* *********************************************************************************************************************************************
Output precision */
         else if(strcmp(compare,"Output precision [DOUBLE,LONG DOUBLE]=")==0){
            /* Extract the output precision for the long double data type */
            while(((ispunct(string[i+1]))!=0)&&(string[i+1]!='-')){i++;} /* Skip all punctuation (except '-') */
            k=0;reset_string(compare);
            do{ /* Copy everything after '=' to '\n' to compare string,
               ignoring punctuation (except fullstop and underscore) and spaces, tabs, newlines etc.. */
            ...
            LDBLP=atoi(compare); /* string to int conversion */
            /* Check LDBLP value */
            if((LDBLP<0)||(LDBLP>LDBL_DIG)){
               reset_string(string);
               sprintf(string,"The long double output precision MUST be in the range 0 to %d",LDBL_DIG);
               print_error("Output precision description error",string,"Please check the \'control.input\' file");
            }
            if(LDBLP==0){ /* if set to ZERO then set equal to the maximum */
               LDBLP=LDBL_DIG;
            }
         }
/* *********************************************************************************************************************************************
Number of OpenMP threads */
         else if(strcmp(compare,"OpenMP threads=")==0){
            while(((ispunct(string[i+1]))!=0)&&(string[i+1]!='-')){i++;} /* Skip all punctuation (except '-') */
            k=0;reset_string(compare);
            do{ /* Copy everything after '=' to '\n' to compare string,
               ignoring punctuation (except fullstop and minus) and spaces, tabs, newlines etc.. */
            ...
            openmp_threads=atoi(compare); /* string to int conversion */
         }
         else{ /* Print error */
            j=0;
...
/* Reads in the long double data from the file pointed to by *file_ptr (ignoring comments) to the array
   ...
   long_double_malloc(&(*structure_ptr).list,(size_t)count); /* Allocate memory to store the data */
   ...
   *file_ptr=fopen(file_string,"r"); /* Reopen the data file for reading */
   ...
   while((fgets(string,sizeof(string),*file_ptr))!=NULL){
      if(string[0]=='#'){continue;} /* Ignore comments */
         ...
         ((*structure_ptr).list)[count]=(long double)atof(string); /* string to long double conversion */
         if((strcmp(data_string,"Wavelength")==0)||(strcmp(data_string,"Effective radius")==0)){
            /* Wavelength and effective radius values must be > 0 */
            if(((*structure_ptr).list)[count]<LDBL_EPSILON){ /* Input data MUST be > 0 */
               reset_string(string);
               ...
               sprintf(string2,"([%d:%.*Lg] <= 0) %s data MUST be > 0",count,LDBL_DIG,((*structure_ptr).list)[count],data_string);
            ...
            if(((*structure_ptr).list)[count]<0.0L){ /* Input data MUST be positive */
               reset_string(string);
               reset_string(string1);
               reset_string(string2);
               sprintf(string,"Please check the file %s",file_string);
         ...
         if(count==0){ /* Set initial value of previous */
            ...
            if((previous>((*structure_ptr).list)[count])||((((*structure_ptr).list)[count]-previous)<LDBL_EPSILON)){
               ...
               sprintf(string,"Please check the file %s",file_string);
               sprintf(string1,"%s input error",data_string);
               sprintf(string2,"([%d:%.*Lg] >= [%d:%.*Lg]) %s data MUST be monotonically increasing",count-1,LDBL_DIG,((*structure_ptr).list)[count-1],count,LDBL_DIG,((*structure_ptr).list)[count],data_string);
               print_error(string1,string2,string);
            }
...
/* Reads in the wavelength and complex refractive index data from the file
   ...
   int a,i,k,count=0;
   long double previous; /* Check if wavelength data is monotonically increasing */
   ...
   long_double_malloc(&(*structure_ptr).wavelengths[material],(size_t)count); /* Allocate memory to store the wavelength data */
   ldcomplex_malloc(&(*structure_ptr).refractive_indices[material],(size_t)count); /* Allocate memory to store the complex refractive index data */
   ...
   *file_ptr=fopen(file_string,"r"); /* Reopen the data file for reading */
   ...
   while((fgets(string,sizeof(string),*file_ptr))!=NULL){
      if(string[0]=='#'){continue;} /* Ignore comments */
         ...
         ((*structure_ptr).wavelengths[material])[count]=(long double)atof(compare); /* string to long double conversion */
         if(((*structure_ptr).wavelengths[material])[count]<LDBL_EPSILON){ /* Wavelength data MUST be positive */
            reset_string(string);
            reset_string(compare);
            sprintf(compare,"([%d:%.*Lg] <= 0) The wavelength data MUST be > 0",count,LDBL_DIG,((*structure_ptr).wavelengths[material])[count]);
         ...
         if(count==0){ /* Set initial value of previous */
            ...
            if((previous>((*structure_ptr).wavelengths[material])[count])||((((*structure_ptr).wavelengths[material])[count]-previous)<LDBL_EPSILON)){
               /* If data is not monotonically increasing */
               reset_string(string);
               reset_string(compare);
               sprintf(string,"Please check the file INPUT_ref_index_vs_wavelength_material_%d.input",material);
               sprintf(compare,"([%d:%.*Lg] >= [%d:%.*Lg]) Wavelength data MUST be monotonically increasing",count-1,LDBL_DIG,((*structure_ptr).wavelengths[material])[count-1],count,LDBL_DIG,((*structure_ptr).wavelengths[material])[count]);
               print_error("Complex refractive index vs wavelength input error",compare,string);
            }
         ...
         for(a=0;a<2;a++){ /* a=0 real component,a=1 imaginary component */
            ...
            while(((ispunct(string[i+1])!=0)&&(string[i+1]!='-'))||(string[i+1]=='i')||(string[i+1]=='I')){i++;} /* Skip 'i', 'I' and all punctuation (except '-') */
            ((*structure_ptr).refractive_indices[material])[count].dat[a]=(long double)atof(compare); /* string to long double conversion */
         }
         count++; /* Next entry */
      }
      reset_string(string);
