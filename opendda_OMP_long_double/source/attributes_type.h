...
/* flag: for each material 'j',j=0..(number_of_dielectric_materials-1), the flag
   ...
   **refractive_indices: if (flag[j]=1) then the arrays wavelengths[j] and refractive_indices[j]
   ...
   ldcomplex *value;
   long double **wavelengths;
   ldcomplex **refractive_indices;
} attributes_refractive_index;
#endif
...
/* Type definition for a structure to hold the target description
   ...
   long double dipoles_per_wavelength; /* number of dipoles per wavelength */
   long double dipole_spacing; /* inter-dipole spacing */
...
/* Type definition for a structure to control incident polarisation
   ...
   long double n_inc[3]; /* Incident direction */
...
#ifndef _OPENDDA_OMPCATTERING_VECTORS_
#define _OPENDDA_OMPCATTERING_VECTORS_
typedef struct {
/* Type definition for a structure to control scattering vectors
   ...
   theta_sca: current scattering vector parallel to the
              scattering plane */
   long double n_sca[3]; /* Scattering direction */
   long double phi_sca[3]; /* Scattering vector perpendicular to the scattering plane */
   long double theta_sca[3]; /* Scattering vector parallel to the scattering plane */
} attributes_scattering_vectors;
#endif

/* Cross section type definition */
#ifndef _OPENDDA_CROSS_OMPECTIONS_
#define _OPENDDA_CROSS_OMPECTIONS_
typedef struct {
/* Type definition for a structure to control cross sections
   ...
   long double extinction; /* Extinction cross section */
   long double absorption; /* Absorption cross section */
   long double scattering; /* Scattering cross section */
   long double geometrical; /* Geometrical cross section */
} attributes_cross_sections;
#endif
...
/* Type definition for a structure to control efficiencies
   ...
   long double extinction; /* Extinction efficiency */
   long double absorption; /* Absorption efficiency */
   long double scattering; /* Scattering efficiency */
...
/* Type definition for a structure to control timing
   ...
   zdft_zidft_and_fd_mult: Time for the DFT of the 3 vector components, the
                           ...
                           of the resultant                           
   norm_and_diag: Time for the normalisation of the resultant of the iDFT + 
                                   Time to include the diagonal contributions
   compress: Time to compress the data to occupied sites only
