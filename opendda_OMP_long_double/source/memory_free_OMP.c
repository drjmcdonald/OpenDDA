...
void memory_free_OMP(void){
   ...
   for(i=0;i<openmp_threads;i++){
      ldcomplex_fftwl_free(&xdft[i]);
      ldcomplex_fftwl_free(&ydft[i]);
      ldcomplex_fftwl_free(&zdft[i]);
   }
   ldcomplex_fftwl_free_ptr(&xdft);  /* xdft[openmp_threads][Kpp] */
   ldcomplex_fftwl_free_ptr(&ydft);  /* ydft[openmp_threads][Jpp] */
   ldcomplex_fftwl_free_ptr(&zdft);  /* zdft[openmp_threads][Ppp] */

   /* Scratch arrays for the DFT, FD multiplication & iDFT of the 3 zero-padded vector components [1 for each thread] */
   for(i=0;i<openmp_threads;i++){
      ldcomplex_fftwl_free(&xzscratch[i]);
   }
   ldcomplex_fftwl_free_ptr(&xzscratch);  /* xzscratch[openmp_threads][3*Qpp] */
