...
void fcomplex_mlbicgstab_orig_MPI(void){
   ...
   fcomplex alpha,rho,beta,tempdc,reduce;
...
/* 13.  zd=u,zg=r,zomega=0 */
         ...
         memcpy(zg,r,(size_t)parallel.alloc_vector3*sizeof(fcomplex));
...
/* 18. beta=(-q[0]^{H}(r+rho*zomega))/(rho*c[iterative.vec-1]) */
         ...
         reduce=zero;
         for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
            reduce=fcomplex_add(reduce,fcomplex_mul(fcomplex_conj(q[index0i]),fcomplex_add(r[index0i],fcomplex_mul(rho,zomega[index0i]))));
         }
         MPI_Allreduce(&reduce,&beta,1,fcomplex_type,add_fcomplex,MPI_COMM_WORLD);
