...
void fcomplex_tfqmr_MPI(void){
   ...
   fcomplex eta={{0.0F,0.0F}},eta0,rho,rhok,sigma,alpha,beta,tempdc,*ptr,reduce;
...
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 2. omega=yb=r */
   memcpy(omega,r,(size_t)parallel.alloc_vector3*sizeof(fcomplex));
   memcpy(yb,r,(size_t)parallel.alloc_vector3*sizeof(fcomplex));
...
/* 8. rhok=rtilde^{T}r */
   reduce=zero;
   for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
      reduce=fcomplex_add(reduce,fcomplex_mul(rtilde[index0i],r[index0i]));
   }
...
/* 9. sigma=rtilde^{T}v */
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=fcomplex_add(reduce,fcomplex_mul(rtilde[index0i],v[index0i]));
      }
      MPI_Allreduce(&reduce,&sigma,1,fcomplex_type,add_fcomplex,MPI_COMM_WORLD);
...
/* 17. d=ya+(theta0^{2}*eta0/alpha)*d */
         tempdc=fcomplex_scale(fcomplex_div(eta0,alpha),theta0*theta0);
         for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
            d[index0i]=fcomplex_add(ya[index0i],fcomplex_mul(tempdc,d[index0i]));
         }
...
/* 21. rhok=rtilde^{T}omega */
      ...
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=fcomplex_add(reduce,fcomplex_mul(rtilde[index0i],omega[index0i]));
      }
      MPI_Allreduce(&reduce,&rhok,1,fcomplex_type,add_fcomplex,MPI_COMM_WORLD);
      /* Check for iterative.breakdown */
