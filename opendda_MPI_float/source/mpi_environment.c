...
/* Create an MPI data type for fcomplex */
   MPI_Type_contiguous(2,MPI_FLOAT,&fcomplex_type);
   MPI_Type_commit(&fcomplex_type); /* Commit the datatype */

/* Create a user-defined MPI function to add complex numbers */
   MPI_Op_create((MPI_User_function*)fcomplex_sum,1,&add_fcomplex);
}

void mpi_finalise(void){

/* Free the MPI data type for fcomplex */
   MPI_Type_free(&fcomplex_type);

/* Free the user-defined MPI function to add complex numbers */
   MPI_Op_free(&add_fcomplex);

   MPI_Finalize();
}

int fcomplex_sum(fcomplex *in,fcomplex *inout,int *len,MPI_Datatype *dtptr){
/* Function to add fcomplex for MPI_Allreduce */
