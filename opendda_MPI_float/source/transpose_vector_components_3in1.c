...
/* Create vector to send and receive all 3 zero-padded vector components at the same time */
   MPI_Type_vector(count3,blocklen3,stride3,fcomplex_type,&vector3);
...
void transpose_vector_component_3in1_1(int before,int after,int planesbefore,int planesafter,int recv){
   ...
   MPI_Aint hstride3=sizeof(fcomplex)*parallel.alloc_padded;
...
/* Create vector to send all 3 zero-padded vector components at the same time */
   MPI_Type_vector(3,blocklen3,stride3,fcomplex_type,&vector_send3);
...
/* Buffering requirement is the bufsize to store the accumulated planes and `np' planes for the dynamic
   ...
   bufsize+=parallel.np;
   fcomplex_malloc(&buffer,(size_t)(bufsize*planesbefore*target.K*3)); /* Allocate the buffer */
...
/* Send the locally transposed data to the appropriate processors */
   ...
   while(i<before||i0<before||index0<planesafter){
      ...
      for(j=0;j<parallel.np;j++){ /* Perform the sends */
         if(i0<before){
            MPI_Isend(&buffer[(i0%bufsize)*index1*3],index1*3,fcomplex_type,i0%parallel.np,parallel.myrank*before+i0,MPI_COMM_WORLD,&rqsend[cs]);
