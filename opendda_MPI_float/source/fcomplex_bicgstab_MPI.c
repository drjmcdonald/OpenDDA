...
void fcomplex_bicgstab_MPI(void){

   int vc,iteration=0;
   
   long int index0i,array0,array1,array2;
   fcomplex rho,rhok,alpha,omega,beta,denom,numer,reduce;
...
/* 9. alpha=rhok/(conj(rtilde^{T})v) */
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=fcomplex_add(reduce,fcomplex_mul(fcomplex_conj(rtilde[index0i]),v[index0i]));
      }
...
/* t is stored in zero_padded_vector */
      ...
      for(vc=0;vc<3;vc++){
         ...
         array1=vc*parallel.alloc_padded;
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            reduce=fcomplex_add(reduce,fcomplex_mul(fcomplex_conj(zero_padded_vector[array1+index0i]),s[array0+index0i]));
         }
      }
      MPI_Allreduce(&reduce,&numer,1,fcomplex_type,add_fcomplex,MPI_COMM_WORLD);
      reduce=zero;denom.dat[1]=0.0F;
      for(vc=0;vc<3;vc++){
         array1=vc*parallel.alloc_padded;
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            reduce.dat[0]+=fcomplex_abs2(zero_padded_vector[array1+index0i]);
         }
      }
      MPI_Allreduce(&reduce.dat[0],&denom.dat[0],1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
      /* Check for iterative.breakdown */
