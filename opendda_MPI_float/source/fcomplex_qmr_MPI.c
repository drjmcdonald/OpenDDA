...
/* r=M^{-1}r */
   ...
   /* omegatilde=vtilde=r */
   memcpy(omegatilde,r,(size_t)parallel.alloc_vector3*sizeof(fcomplex));
   memcpy(vtilde,r,(size_t)parallel.alloc_vector3*sizeof(fcomplex));
...
/*   gamma0=fcomplex_2vectornorm(vtilde);
   ...
   reduce=zero;
   for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
      reduce=fcomplex_add(reduce,fcomplex_mul(omegatilde[index0i],vtilde[index0i]));
   }
...
/* interaction_matrix*omegatilde=M^{-1}interaction_matrix*omegatilde */
   ...
   for(vc=0;vc<3;vc++){
      ...
      array1=vc*parallel.alloc_padded;
      for(index0i=0;index0i<parallel.alloc_vector;index0i++){
         reduce=fcomplex_add(reduce,fcomplex_mul(zero_padded_vector[array1+index0i],vtilde[array0+index0i]));
      }
   }
   MPI_Allreduce(&reduce,&epsilon,1,fcomplex_type,add_fcomplex,MPI_COMM_WORLD);
