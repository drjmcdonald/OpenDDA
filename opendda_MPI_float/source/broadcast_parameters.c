...
void broadcast_control_parameters(void){
   ...
   MPI_Datatype types[26]={MPI_INT,MPI_INT,MPI_CHAR,MPI_INT,fcomplex_type,fcomplex_type,MPI_CHAR,
                  MPI_INT,MPI_FLOAT,MPI_FLOAT,MPI_INT,MPI_INT,MPI_INT,
   ...
   struct broadcast_control{
      ...
      float iterative_tolerance; /* convergence tolerance */
      float iterative_breakdown; /* breakdown tolerance */
      ...
      float wavelength_limits[3]; /* wavelength limits */
      float radius_limits[3]; /* radius limits */
      float euler_phi_limits[3]; /* Euler phi limits */
      float euler_theta_limits[3]; /* Euler theta limits */
      float euler_psi_limits[3]; /* Euler psi limits */
      float theta_limits[3]; /* theta limits */
      float phi_limits[3]; /* phi limits */
      int timing_enabled; /* timing flag */
      int DBLP; /* double output precision */
      int FLTP; /* float output precision */
   ...
   MPI_Address(&control.FLTP,&displacements[23]);
   ...
   if(parallel.myrank==0){ /* Restrict to master */
      ...
      control.FLTP=FLTP; /* float output precision */
   ...
   if(parallel.myrank!=0){ /* Restrict to slaves */
      ...
      FLTP=control.FLTP; /* float output precision */
...
void broadcast_physical_parameters(void){
   ...
   int i,count=8;
   int blocklens[8];
   MPI_Datatype types[8]={MPI_FLOAT,MPI_FLOAT,MPI_FLOAT,
               MPI_FLOAT,MPI_FLOAT,MPI_FLOAT,
               MPI_FLOAT,fcomplex_type};
   ...
   struct broadcast_physical{
      ...
      float wavenumber;
      float radius_value;
      float target_dipole_spacing;
      float euler_phi_value;
      float euler_theta_value;
      float euler_psi_value;
      fcomplex *refractive_index_value;
   };
   struct broadcast_physical physical;

   /* Allocate memory to store the current refractive index values */
   fcomplex_malloc(&physical.refractive_index_value,(size_t)refractive_index.number_of_materials);
   ...
   fcomplex_free(&physical.refractive_index_value);
}
