...
/* The negative ZYZ vector rotation matrix R(-psi)R(-theta)R(-phi)=
   ...
   float cosphi,sinphi,theta,sintheta,cospsi,sinpsi,rotation[9];

   /* Euler theta is distributed in cos(theta) */
   theta=radians_to_degrees(acosf(costheta));
   cosphi=cosf(degrees_to_radians(phi)); /* Convert phi to radians and calculate the cosine */
   sinphi=sinf(degrees_to_radians(phi)); /* Convert phi to radians and calculate the sine */
   sintheta=sinf(degrees_to_radians(theta)); /* Convert theta to radians and calculate the sine */
   cospsi=cosf(degrees_to_radians(psi)); /* Convert psi to radians and calculate the cosine */
   sinpsi=sinf(degrees_to_radians(psi)); /* Convert psi to radians and calculate the sine */
...
/* Rotate the incident vectors LF to TF */
   ...
   incident_TF.polarisation[0]=fcomplex_add(fcomplex_scale(incident_LF.polarisation[0],rotation[0]),
                                 fcomplex_scale(incident_LF.polarisation[1],rotation[1])); /* e0x in the target frame */
   incident_TF.polarisation[1]=fcomplex_add(fcomplex_scale(incident_LF.polarisation[0],rotation[3]),
                                 fcomplex_scale(incident_LF.polarisation[1],rotation[4])); /* e0y in the target frame */
   incident_TF.polarisation[2]=fcomplex_add(fcomplex_scale(incident_LF.polarisation[0],rotation[6]),
                                 fcomplex_scale(incident_LF.polarisation[1],rotation[7])); /* e0z in the target frame */
   ...
   incident_TF.orthonormal[0]=fcomplex_add(fcomplex_scale(incident_LF.orthonormal[0],rotation[0]),
                              fcomplex_scale(incident_LF.orthonormal[1],rotation[1])); /* e1x in the target frame */
   incident_TF.orthonormal[1]=fcomplex_add(fcomplex_scale(incident_LF.orthonormal[0],rotation[3]),
                              fcomplex_scale(incident_LF.orthonormal[1],rotation[4])); /* e1y in the target frame */
   incident_TF.orthonormal[2]=fcomplex_add(fcomplex_scale(incident_LF.orthonormal[0],rotation[6]),
                              fcomplex_scale(incident_LF.orthonormal[1],rotation[7])); /* e1z in the target frame */
...
|_ n_{sca}x     n_{sca}y     n_{sca}z _| |_cos(phi)sin(theta) sin(phi)sin(theta)  cos(theta)_| */
   ...
   cosphi=cosf(degrees_to_radians(phi)); /* Convert phi to radians and calculate the cosine */
   sinphi=sinf(degrees_to_radians(phi)); /* Convert phi to radians and calculate the sine */
   costheta=cosf(degrees_to_radians(theta)); /* Convert theta to radians and calculate the cosine */
   sintheta=sinf(degrees_to_radians(theta)); /* Convert theta to radians and calculate the sine */
   ...
   scattering_LF.phi_sca[2]=0.0F; /* phi_{sca}z in the lab frame */
