...
void dcomplex_bicg_sym_S(void){

   int vc,iteration=0;
   long int index0i,array0,array1,array2;
   dcomplex rho,rhok,alpha,beta,delta;
...
/* 3. rhok=r^{T}r */
      rhok=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         rhok=dcomplex_add(rhok,dcomplex_mul(r[index0i],r[index0i]));
      }
...
/* interaction_matrix*p is stored in zero_padded_vector */
      ...
      for(vc=0;vc<3;vc++){
         ...
         array1=vc*target.Nv;
         for(index0i=0;index0i<target.Nd;index0i++){
            alpha=dcomplex_add(alpha,dcomplex_mul(p[array0+index0i],zero_padded_vector[array1+index0i]));
         }
      }
      /* Check for iterative.breakdown */
