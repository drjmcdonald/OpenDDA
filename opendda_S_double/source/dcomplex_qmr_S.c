...
/*   gamma0=dcomplex_2vectornorm(vtilde);
   ...
   rho=zero;
   for(index0i=0;index0i<target.Nd3;index0i++){
      rho=dcomplex_add(rho,dcomplex_mul(omegatilde[index0i],vtilde[index0i]));
   }
...
/* interaction_matrix*omegatilde=M^{-1}interaction_matrix*omegatilde */
   ...
   for(vc=0;vc<3;vc++){
      ...
      array1=vc*target.Nv;
      for(index0i=0;index0i<target.Nd;index0i++){
         epsilon=dcomplex_add(epsilon,dcomplex_mul(zero_padded_vector[array1+index0i],vtilde[array0+index0i]));
      }
   }
