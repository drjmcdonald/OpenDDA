...
/* degree   degree of the polynomials for the interpolating function
   ...
   double first_derivative,primary_estimate,volatility_factor,distance_factor,weighting_factor;
   double sum_primary_estimate_finite,sum_primary_estimate_infinite,final_estimate,final_estimate0,slope;
   double sum_weighting_factor_finite,sum_weighting_factor_infinite,denom,d0,d1,d2,temp;
   double x10,x13,x20,x21,x30,x32,b0,b1,v0,v1,v2,v3,sx,sy,sxy,sxs,sx2,a0,a1,a2,a3,u,v,a,b,c,d,e,f;
   char string[100];
...
/* Sanity checks */
   ...
   for(i=0;i<(size-1);i++){ /* Check to ensure that the x data is sequentially increasing and that xdata[i+1]!=xdata[i] */
      if((xdata[i]>xdata[i+1])||((xdata[i+1]-xdata[i])<DBL_EPSILON)){
         sprintf(string,"Improved Akima Error: %.*g>=%.*g",DBLP,xdata[i],DBLP,xdata[i+1]);
...
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
   if(size>4){ /* General case - 5 points or more */
      ...
      for(i=0;i<points;i++){ /* Cycle through the points for which interpolation is desired */
         right=size-1;
         if((xinterp[i]-xdata[0])<DBL_EPSILON){ /* Point <= Range minimum */
            interval=0;
         }
...
/* The final estimate of the first derivative of the interpolating function is
   ...
   weighting_factor=1/[V*D] if V~0 then the weight is infinite otherwise it is finite */
                  sum_primary_estimate_finite=0.0; /* Initalise value to zero */
                  sum_weighting_factor_finite=0.0; /* Initalise value to zero */
                  sum_primary_estimate_infinite=0.0; /* Initalise value to zero */
                  sum_weighting_factor_infinite=0.0; /* Initalise value to zero */
...
/* Calculate the volatility factor V, the distance factor D
   ...
   distance_factor=d0*d0+d1*d1+d2*d2 */
                        ...
                        sx2=sx*sx; /* (sum[x])^{2} */
                        denom=4.0*sxs-sx2; /* 4*sum[x^{2}]-(sum[x])^{2} */
                        b0=(sxs*sy-sx*sxy)/denom; /* b0=(sum[x^{2}]sum[y]-sum[x]sum[y])/(4*sum[x^{2}]-sum[x]^{2}) */
                        b1=(4.0*sxy-sx*sy)/denom; /* b1=(4*sum[xy]-sum[x]sum[y])/(4*sum[x^{2}]-sum[x]^{2}) */
...
/* Check whether the volatility factor is essentially zero */
                        if(volatility_factor>((ydata[id0].dat[part]*ydata[id0].dat[part]+
                                                ...
                                                ydata[id3].dat[part]*ydata[id3].dat[part])*DBL_EPSILON)){
                           /* Finite weight */
                           weighting_factor=1.0/(volatility_factor*distance_factor);
                           sum_primary_estimate_finite+=primary_estimate*weighting_factor;
                           sum_weighting_factor_finite+=weighting_factor;
                        }
                        else{
                           /* Infinite weight */
                           sum_primary_estimate_infinite+=primary_estimate;
                           sum_weighting_factor_infinite+=1.0;
                        }
...
/* Calculate the final estimate of the first derivative of the interpolating function is
   ...
   [Equation 14] y'_{i}=[y'_{imm}w_{imm}+y'_{im}w_{im}+y'_{ip}w_{ip}+y'_{ipp}w_{ipp}]/[w_{imm}+w_{im}+w_{ip}+w_{ipp}] */
                     if(sum_weighting_factor_infinite<0.5){ /* If no infinite weights exist */
...
/* Calculate the coefficients of the 3rd-degree polynomial
   ...
   [Equation 5] m_{i}=(y_{i+1}-y_{i})/(x_{i+1}-x_{i}) */
                  ...
                  a1=final_estimate0;
                  a2=-((2.0*(final_estimate0-slope)+(final_estimate-slope))/denom);
...
/* The nth-degree polynomial is represented by
   ...
   [Equation 5] m_{i}=(y_{i+1}-y_{i})/(x_{i+1}-x_{i}) */
                  ...
                  denom=(double)(degree*(degree-2)); /* n(n-2) */
...
/* Subcase 1: 2 data points - Linear interpolation and extrapolation
   ...
   a=(x-x0)/(x1-x0) */
      for(i=0;i<points;i++){ /* Cycle through the points for which interpolation is desired */
         ...
         yinterp[i].dat[part]=(1.0-a)*ydata[0].dat[part]+a*ydata[1].dat[part];
      }
   }
   else if(size==3){
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* Subcase 2: 3 data points - Quadratic interpolation and linear extrapolation
   ...
   y=y0+m(x-x0) where m is the slope i.e. first derivative */
      for(i=0;i<points;i++){ /* Cycle through the points for which interpolation is desired */
         ...
         if(xinterp[i]<(xdata[0]+DBL_EPSILON)){ /* Point <= Range minimum - Linear extrapolation */
/* first_derivative=F(0,1,2)=[f1 + f2]/[f3 + f4]
   ...
   f4=x10*x10*x20; */
         ...
         else if(xinterp[i]<(xdata[size-1]-DBL_EPSILON)){ /* Range minimum < Point < Range maximum - Quadratic interpolation */
            yinterp[i].dat[part]=ydata[0].dat[part]*c*(1.0-a)+ydata[1].dat[part]*a*(1.0-b)+ydata[2].dat[part]*b*(1.0-c);
         }
...
/* first_derivative=F(0,1,2,3)=[f1 + f2 + f3]/[f4]
   ...
   f4=x10*x20*x30*x21*x32*-x13; */
         ...
         else if(xinterp[i]<(xdata[size-1]-DBL_EPSILON)){ /* Range minimum < Point < Range maximum - Cubic interpolation */
            yinterp[i].dat[part]=ydata[0].dat[part]*c*e*(1.0-a)+ydata[1].dat[part]*a*d*(1.0-b)+
                        ydata[2].dat[part]*b*f*(1.0-c)+ydata[3].dat[part]*(1.0-d)*(1.0-e)*(1.0-f);
         }
         else{ /* Point >= Range maximum - Linear extrapolation */
/* first_derivative=F(size-1,size-2,size-3,size-4)=F(0,1,2,3)=[f1 + f2 + f3]/[f4]
