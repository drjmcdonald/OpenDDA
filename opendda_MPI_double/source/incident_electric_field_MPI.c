...
void incident_electric_field_MPI(void){
   ...
   double wave_vector_TF[3],kr0,kr1,kr2;
   ...
   for(k=1;k<=target.P;k++){
      if((k-1)%parallel.np==parallel.myrank){ /* Plane k is on proc (k%np) */
         ...
         kr2=wave_vector_TF[2]*((double)k); /* kz*rz */
         for(j=1;j<=target.J;j++){
            index0j=index0k+(j-1)*target.K;
            kr1=kr2+wave_vector_TF[1]*((double)j); /* ky*ry+kz*rz */
            for(i=1;i<=target.K;i++){
               index0i=index0j+(i-1);
               kr0=kr1+wave_vector_TF[0]*((double)i); /* k.r=kx*rx+ky*ry+kz*rz */
               ikr.dat[0]=0.0;ikr.dat[1]=kr0; /* ikr */
               ...
               if(target.occupied[(int)((double)index0i/32.0)]&(1<<(index0i%32))){ /* Lattice site is occupied */
                  for(vc=0;vc<3;vc++){
                     incident_E_field[element+vc*parallel.alloc_vector]=dcomplex_mul(eikr,current_polarisation_TF[vc]);
                  }
                  element++;
               }
            }
