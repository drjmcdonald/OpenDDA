...
void dcomplex_qmr_MPI(void){
   ...
   dcomplex epsilon,mu={{0.0,0.0}},tau,tau0,reduce;
   double ratio,norm0,norm1,gamma0,gamma1,xi,xi0,lambda=1.0;
   double theta=-1.0,denom,tempd,terminate,temp,result,start,finish;
...
/* r=M^{-1}r */
   ...
   /* omegatilde=vtilde=r */
   memcpy(omegatilde,r,(size_t)parallel.alloc_vector3*sizeof(dcomplex));
   memcpy(vtilde,r,(size_t)parallel.alloc_vector3*sizeof(dcomplex));
...
/*   gamma0=dcomplex_2vectornorm(vtilde);
   ...
   reduce=zero;
   for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
      reduce=dcomplex_add(reduce,dcomplex_mul(omegatilde[index0i],vtilde[index0i]));
   }
...
/* interaction_matrix*omegatilde=M^{-1}interaction_matrix*omegatilde */
   ...
   for(vc=0;vc<3;vc++){
      ...
      array1=vc*parallel.alloc_padded;
      for(index0i=0;index0i<parallel.alloc_vector;index0i++){
         reduce=dcomplex_add(reduce,dcomplex_mul(zero_padded_vector[array1+index0i],vtilde[array0+index0i]));
      }
   }
   MPI_Allreduce(&reduce,&epsilon,1,dcomplex_type,add_dcomplex,MPI_COMM_WORLD);
