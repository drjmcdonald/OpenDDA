...
/* Treat (proc,element)=(myrank,i) as a source and work out the destination */
         ...
         MPI_Isend(&interaction_matrix[comp*parallel.alloc_tensor+(i*target.Ka)],target.Ka,dcomplex_type,j1%parallel.np,k1*before+j1,MPI_COMM_WORLD,&rqsend); /* Non-blocking send */
...
/* Treat (proc,element)=(myrank,i) as a destination and work out the source */
         ...
         MPI_Irecv(&interaction_matrix[comp*parallel.alloc_tensor+(i*target.Ka)],target.Ka,dcomplex_type,k%parallel.np,k*before+j,MPI_COMM_WORLD,&rqrecv[cr]); /* Non-blocking receive */
         cr++;
      }
   }
...
/* Send the locally transposed data to the appropriate processors */
   ...
   while((i<before)||(index0<planesafter)){
      ...
      while((i*planesbefore)<((index0+1)*after)&&(i<before)){ /* Send the planes that are impeding the in-place receive.
         ...
         MPI_Isend(&interaction_matrix[comp*parallel.alloc_tensor+(i*index1)],index1,dcomplex_type,i%parallel.np,index2+i,MPI_COMM_WORLD,&rqsend[cs]);
         cs++;i++;
      }
...
/* Buffering requirement is the bufsize to store the accumulated planes and
   ...
   bufsize+=parallel.np;
   dcomplex_malloc(&buffer,(size_t)(bufsize*planesbefore*target.Ka)); /* Allocate the buffer */
...
/* Send the locally transposed data to the appropriate processors */
   ...
   while(i<before||i0<before||index0<planesafter){
      ...
      for(j=0;j<parallel.np;j++){ /* Perform the sends */
         if(i0<before){
            MPI_Isend(&buffer[(i0%bufsize)*index1],index1,dcomplex_type,i0%parallel.np,parallel.myrank*before+i0,MPI_COMM_WORLD,&rqsend[cs]);
