...
void dcomplex_tfqmr_MPI(void){

   int m,vc,iteration=0,exitflag=0;
   long int index0i,array0,array1,array2;
   dcomplex eta={{0.0,0.0}},eta0,rho,rhok,sigma,alpha,beta,tempdc,*ptr,reduce;
   double ratio,norm0,norm1,tau,theta=0.0,tau0,theta0,c,kappa,error;
   double terminate,temp,result,start,finish;
...
/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 2. omega=yb=r */
   memcpy(omega,r,(size_t)parallel.alloc_vector3*sizeof(dcomplex));
   memcpy(yb,r,(size_t)parallel.alloc_vector3*sizeof(dcomplex));
...
/* 8. rhok=rtilde^{T}r */
   reduce=zero;
   for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
      reduce=dcomplex_add(reduce,dcomplex_mul(rtilde[index0i],r[index0i]));
   }
...
/* 9. sigma=rtilde^{T}v */
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=dcomplex_add(reduce,dcomplex_mul(rtilde[index0i],v[index0i]));
      }
      MPI_Allreduce(&reduce,&sigma,1,dcomplex_type,add_dcomplex,MPI_COMM_WORLD);
...
/* 17. d=ya+(theta0^{2}*eta0/alpha)*d */
         tempdc=dcomplex_scale(dcomplex_div(eta0,alpha),theta0*theta0);
         for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
            d[index0i]=dcomplex_add(ya[index0i],dcomplex_mul(tempdc,d[index0i]));
         }
...
/* 21. rhok=rtilde^{T}omega */
      ...
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=dcomplex_add(reduce,dcomplex_mul(rtilde[index0i],omega[index0i]));
      }
      MPI_Allreduce(&reduce,&rhok,1,dcomplex_type,add_dcomplex,MPI_COMM_WORLD);
      /* Check for iterative.breakdown */
