...
/* interaction_matrix*v=M^{-1}interaction_matrix*v */
      ...
      for(vc=0;vc<3;vc++){
         ...
         array1=vc*parallel.alloc_padded;
         for(index0i=0;index0i<parallel.alloc_vector;index0i++){
            reduce=dcomplex_add(reduce,dcomplex_mul(v[array0+index0i],zero_padded_vector[array1+index0i]));
         }
      }
...
/* interaction_matrix*v is stored in zero_padded_vector */
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<parallel.alloc_vector;index0i++){
               array2=array0+index0i;
               vtilde[array2]=dcomplex_sub(zero_padded_vector[array1+index0i],dcomplex_add(dcomplex_mul(alpha,v[array2]),dcomplex_mul(vtilde[array2],beta)));
            }
         }
      }
...
/* 12. beta=sqrt[vtilde^{T}vtilde],omega=||vtilde||/|beta| */
      reduce=zero;
      for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
         reduce=dcomplex_add(reduce,dcomplex_mul(vtilde[index0i],vtilde[index0i]));
      }
      MPI_Allreduce(&reduce,&beta,1,dcomplex_type,add_dcomplex,MPI_COMM_WORLD);
      beta=dcomplex_sqrt(beta);
      /* Check for iterative.breakdown */
...
/* 17. p=(v-eta*p-theta*pold)/zeta (tempdc0=1/zeta) */
         for(index0i=0;index0i<parallel.alloc_vector3;index0i++){
            ...
            pold[index0i]=dcomplex_mul(dcomplex_sub(v[index0i],dcomplex_add(dcomplex_mul(eta,p[index0i]),dcomplex_mul(theta,pold[index0i]))),tempdc0);
         }
