...
void broadcast_control_parameters(void){
   ...
   long begin;
   int i,count=25;
   int blocklens[25]={6,1,50,1,3,3,15,1,1,1,1,1,1,1,3,3,3,3,3,3,3,1,1,1,1};
   MPI_Datatype types[25]={MPI_INT,MPI_INT,MPI_CHAR,MPI_INT,dcomplex_type,dcomplex_type,MPI_CHAR,
                  MPI_INT,MPI_DOUBLE,MPI_DOUBLE,MPI_INT,MPI_INT,MPI_INT,
                  MPI_INT,MPI_DOUBLE,MPI_DOUBLE,MPI_DOUBLE,
                  MPI_DOUBLE,MPI_DOUBLE,MPI_DOUBLE,MPI_DOUBLE,
                  MPI_INT,MPI_INT,MPI_INT,MPI_INT};
   MPI_Aint displacements[25];
   ...
   struct broadcast_control{
      ...
      double wavelength_limits[3]; /* wavelength limits */
      double radius_limits[3]; /* radius limits */
      double euler_phi_limits[3]; /* Euler phi limits */
      double euler_theta_limits[3]; /* Euler theta limits */
      double euler_psi_limits[3]; /* Euler psi limits */
      double theta_limits[3]; /* theta limits */
      double phi_limits[3]; /* phi limits */
   ...
   MPI_Address(&control.tensor_transpose,&displacements[23]);
   MPI_Address(&control.padded_transpose,&displacements[24]);
...
void broadcast_physical_parameters(void){
   ...
   struct broadcast_physical{
      double wavelength_value;
      double wavenumber;
      double radius_value;
      double target_dipole_spacing;
      double euler_phi_value;
      double euler_theta_value;
      double euler_psi_value;
      dcomplex *refractive_index_value;
   };
   struct broadcast_physical physical;

   /* Allocate memory to store the current refractive index values */
   dcomplex_malloc(&physical.refractive_index_value,(size_t)refractive_index.number_of_materials);
   ...
   dcomplex_free(&physical.refractive_index_value);
}
