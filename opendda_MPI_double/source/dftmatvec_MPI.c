...
void dftmatvec_MPI(dcomplex *vector){
   ...
   double isign,ksign,grid_points,start,finish;
         ...
         for(k=0;k<target.P;k++){
            if(k%parallel.np==parallel.myrank){ /* Plane k is on proc (k%np) */
               ...
               for(i=0;i<target.Ka;i++){ /* Make circulant */
                ...
                /* Execute yDFT */
               fftw_execute_dft(plan_tcydft,(fftw_complex *)(&interaction_matrix[index0k]),(fftw_complex *)(&interaction_matrix[index0k]));
            }
         }
      }
      if(timing.enabled){ /* Finish timing and add contribution */
