...
void dipole_polarisabilities_S(void){

   int vc,material,shift;
   long int array0,index0i,element;
   float c1,c2,c3,b1,b2,b3,kd,d3,S;
   fcomplex m2,current_polarisation[3],term0,term1,alpha_cm,temp;

   c1=-5.942421945054091F;
   c2=0.517881857406567F;
   c3=4.006974687341422F;
   b1=c1/fpi;
   b2=c2/fpi;
   b3=-(3.0F*c2+c3)/fpi;
...
/* b1=-1.8915316F;
   ...
   S=0.0F;
   for(vc=0;vc<3;vc++){
      S+=(incident_TF.n_inc[vc]*fcomplex_abs(current_polarisation[vc]))*
         (incident_TF.n_inc[vc]*fcomplex_abs(current_polarisation[vc]));
   }
   ...
   if(refractive_index.number_of_materials>1){ /* Composition array is only used
      ...
      term1=fcomplex_scale(onei,((2.0F/3.0F)*kd*kd*kd)); /* (2/3)i(kd)^{3} */
      for(vc=0;vc<3;vc++){
         element=0;
         array0=vc*target.Nd;
         for(index0i=0;index0i<target.N;index0i++){
            if(target.occupied[(int)((double)index0i/32.0)]&(1<<(index0i%32))){ /* Lattice site is occupied */
               ...
               m2=fcomplex_mul(refractive_index.value[material],refractive_index.value[material]); /* m^{2} */
               alpha_cm=fcomplex_scale(fcomplex_div(fcomplex_sub(m2,one),
                        fcomplex_add(m2,two)),((3.0F*d3)/(4.0F*fpi))); /* Clausius-Mossotti polarisability */
               term0=fcomplex_scale(fcomplex_add(fcomplex_scale(one,b1),
                        fcomplex_scale(m2,(b2+b3*S))),(kd*kd)); /* [b1+(b2+b3*S)m^{2}](kd)^{2} */
               /* alpha_LDR=alpha_cm/(1+[alpha_cm/d^{3}][(b1+m^{2}b2+m^{2}b3S)(kd)^{2}-(2/3)i(kd)^{3})
               ...
               interaction_matrix_diagonal[array0+element]=fcomplex_div(fcomplex_add(one,
                     fcomplex_mul(fcomplex_scale(alpha_cm,(1.0F/d3)),fcomplex_sub(term0,term1))),alpha_cm);
               element++;
            }
         }
      }
      ...
      /* Calculate the Clausius-Mossotti relation, alpha_cm=(3d^{3}(m^{2}-1))/(4pi(m^{2}+2)), where
         m is the complex refractive index */
      m2=fcomplex_mul(refractive_index.value[0],refractive_index.value[0]); /* m^{2} */
      alpha_cm=fcomplex_scale(fcomplex_div(fcomplex_sub(m2,one),
               fcomplex_add(m2,two)),((3.0F*d3)/(4.0F*fpi))); /* Clausius-Mossotti polarisability */
      term0=fcomplex_scale(fcomplex_add(fcomplex_scale(one,b1),
               fcomplex_scale(m2,(b2+b3*S))),(kd*kd)); /* [b1+(b2+b3*S)m^{2}](kd)^{2} */
      /* alpha_LDR=alpha_cm/(1+[alpha_cm/d^{3}][(b1+m^{2}b2+m^{2}b3S)(kd)^{2}-(2/3)i(kd)^{3})
         ...
         1/alpha_LDR=(1+[alpha_cm/d^{3}][(b1+m^{2}b2+m^{2}b3S)(kd)^{2}-(2/3)i(kd)^{3})/alpha_cm */
      temp=fcomplex_div(fcomplex_add(one,fcomplex_mul(fcomplex_scale(alpha_cm,(1.0F/d3)),
                        fcomplex_sub(term0,term1))),alpha_cm);
