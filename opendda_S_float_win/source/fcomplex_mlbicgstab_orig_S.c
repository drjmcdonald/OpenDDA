...
void fcomplex_mlbicgstab_orig_S(void){

   int i,ii,s,t,vc,iteration=0;
   long int index0i,array0,array1,array2;
   fcomplex alpha,rho,beta,tempdc;
...
/* 7. alpha=(q[0]^{H}r)/(c[iterative.vec-1]) */
      ...
      for(index0i=0;index0i<target.Nd3;index0i++){
         alpha=fcomplex_add(alpha,fcomplex_mul(fcomplex_conj(q[index0i]),r[index0i]));
      }
...
/* 18. beta=(-q[0]^{H}(r+rho*zomega))/(rho*c[iterative.vec-1]) */
         ...
         beta=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            beta=fcomplex_add(beta,fcomplex_mul(fcomplex_conj(q[index0i]),fcomplex_add(r[index0i],fcomplex_mul(rho,zomega[index0i]))));
         }
...
/* 28. alpha=(q[ii]^{H}u)/(c[ii-1]) */
            ...
            for(vc=0;vc<3;vc++){
               ...
               array1=ii*target.Nd3+vc*target.Nd;
               for(index0i=0;index0i<target.Nd;index0i++){;
                  alpha=fcomplex_add(alpha,fcomplex_mul(fcomplex_conj(q[array1+index0i]),u[array0+index0i]));
               }
            }
            /* Check for iterative.breakdown */
