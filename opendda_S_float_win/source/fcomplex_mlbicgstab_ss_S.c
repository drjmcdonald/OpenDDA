...
void fcomplex_mlbicgstab_ss_S(void){

   int i,ii,s,t,vc,iteration=0;
   long int index0i,array0,array1,array2;
   fcomplex alpha,rho,tempdc,tempe,beta;
...
/* 5. c[0]=q[0]^{H}omega[0] */
   c[0]=zero;
   for(index0i=0;index0i<target.Nd3;index0i++){
      c[0]=fcomplex_add(c[0],fcomplex_mul(fcomplex_conj(q[index0i]),omega[index0i]));
   }
...
/* tempe=q[ii]^{H}r */
         ...
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<target.Nd;index0i++){
               tempe=fcomplex_add(tempe,fcomplex_mul(fcomplex_conj(q[array1+index0i]),r[array0+index0i]));
            }
         }
...
/* 20. c[ii]=q[ii]^{H}zomega */
         ...
         for(vc=0;vc<3;vc++){
            ...
            array1=ii*target.Nd3+vc*target.Nd;
            for(index0i=0;index0i<target.Nd;index0i++){
               c[ii]=fcomplex_add(c[ii],fcomplex_mul(fcomplex_conj(q[array1+index0i]),zomega[array0+index0i]));
            }
         }
      } /* End ii for loop */
...
/* 25. rho=(-[interaction_matrix*M^{-1}r]^{H}r)/(||interaction_matrix*M^{-1}r||^2)=
         ...
         tempdc=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            tempdc=fcomplex_add(tempdc,fcomplex_mul(fcomplex_conj(zomega[index0i]),zomega[index0i]));
         }
         /* Check for iterative.breakdown */
...
/* rho=[interaction_matrix*M^{-1}r]^{H}r */
         rho=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            rho=fcomplex_add(rho,fcomplex_mul(fcomplex_conj(zomega[index0i]),r[index0i]));
         }
...
/* 34. c[0]=q[0]^{H}zomega */
      c[0]=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         c[0]=fcomplex_add(c[0],fcomplex_mul(fcomplex_conj(q[index0i]),zomega[index0i]));
      }
