...
void dcomplex_mlbicgstab_orig_S(void){
   ...
   dcomplex alpha,rho,beta,tempdc;
...
/* 7. alpha=(q[0]^{H}r)/(c[iterative.vec-1]) */
      ...
      for(index0i=0;index0i<target.Nd3;index0i++){
         alpha=dcomplex_add(alpha,dcomplex_mul(dcomplex_conj(q[index0i]),r[index0i]));
      }
...
/* 13.  zd=u,zg=r,zomega=0 */
         ...
         memcpy(zg,r,(size_t)target.Nd3*sizeof(dcomplex));
...
/* 18. beta=(-q[0]^{H}(r+rho*zomega))/(rho*c[iterative.vec-1]) */
         ...
         beta=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            beta=dcomplex_add(beta,dcomplex_mul(dcomplex_conj(q[index0i]),dcomplex_add(r[index0i],dcomplex_mul(rho,zomega[index0i]))));
         }
...
/* 28. alpha=(q[ii]^{H}u)/(c[ii-1]) */
            ...
            for(vc=0;vc<3;vc++){
               ...
               array1=ii*target.Nd3+vc*target.Nd;
               for(index0i=0;index0i<target.Nd;index0i++){;
                  alpha=dcomplex_add(alpha,dcomplex_mul(dcomplex_conj(q[array1+index0i]),u[array0+index0i]));
               }
            }
            /* Check for iterative.breakdown */
