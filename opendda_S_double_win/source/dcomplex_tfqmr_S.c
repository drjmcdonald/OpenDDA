...
/* 8. rhok=rtilde^{T}r */
   rhok=zero;
   for(index0i=0;index0i<target.Nd3;index0i++){
      rhok=dcomplex_add(rhok,dcomplex_mul(rtilde[index0i],r[index0i]));
   }
...
/* 9. sigma=rtilde^{T}v */
      sigma=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         sigma=dcomplex_add(sigma,dcomplex_mul(rtilde[index0i],v[index0i]));
      }
...
/* 21. rhok=rtilde^{T}omega */
      ...
      rhok=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         rhok=dcomplex_add(rhok,dcomplex_mul(rtilde[index0i],omega[index0i]));
      }
      /* Check for iterative.breakdown */
