...
void dftmatvec_S(dcomplex *vector){
            ...
            for(j=0;j<target.J;j++){
               ...
               for(i=0;i<target.K;i++){ 
                  ...
                  if(i>0){ /* Make circulant */
                     xdft[target.Kpp-i]=dcomplex_scale(interaction_matrix[index0i],xsign[tc]); 
                  }
               }
            ...
            for(i=0;i<target.Ka;i++){
               ...
               for(j=0;j<target.J;j++){
                  ...
                  if(j>0){ /* Make circulant */
                     ydft[target.Jpp-j]=dcomplex_scale(interaction_matrix[index0j],ysign[tc]); 
                  }
               }
            ...
            for(i=0;i<target.Ka;i++){
               ...
               for(k=0;k<target.P;k++){
                  ...
                  if(k>0){ /* Make circulant */
                     zdft[target.Ppp-k]=dcomplex_scale(interaction_matrix[index0k],zsign[tc]);
                  }
               }
...
/* Perform the DFT of the 3 vector components of zero_padded_vector, the
   ...
   for(j=0;j<target.Jpp;j++){
      ...
      for(vc=0;vc<3;vc++){ /* xDFT */
         fftw_execute_dft(plan_vcxdft,(fftw_complex *)(&xzscratch[vc*target.Qpp]),(fftw_complex *)(&xzscratch[vc*target.Qpp]));
      }
      for(vc=0;vc<3;vc++){ /* zDFT */
         fftw_execute_dft(plan_vczdft,(fftw_complex *)(&xzscratch[vc*target.Qpp]),(fftw_complex *)(&xzscratch[vc*target.Qpp]));
      }
      ...
      for(vc=0;vc<3;vc++){ /* ziDFT */
         fftw_execute_dft(plan_vczdfti,(fftw_complex *)(&xzscratch[vc*target.Qpp]),(fftw_complex *)(&xzscratch[vc*target.Qpp]));
      }
      for(vc=0;vc<3;vc++){ /* xiDFT */
         fftw_execute_dft(plan_vcxdfti,(fftw_complex *)(&xzscratch[vc*target.Qpp]),(fftw_complex *)(&xzscratch[vc*target.Qpp]));
      }
