...
void fcomplex_mlbicgstab_S(void){

   int i,ii,s,t,vc,iteration=0;
   long int index0i,array0,array1,array2;
   fcomplex alpha,rho,tempdc,tempd,tempe,tempf,beta;
...
/* 6. c[iterative.vec-1]=q[0]^{H}omega[iterative.vec-1] */
   ...
   for(vc=0;vc<3;vc++){
      ...
      for(index0i=0;index0i<target.Nd;index0i++){
         c[iterative.vec-1]=fcomplex_add(c[iterative.vec-1],fcomplex_mul(fcomplex_conj(q[array0+index0i]),omega[array1+index0i]));
      }
   }
...
/* tempe=q[0]^{H}r */
   tempe=zero;
   for(index0i=0;index0i<target.Nd3;index0i++){
      tempe=fcomplex_add(tempe,fcomplex_mul(fcomplex_conj(q[index0i]),r[index0i]));
   }
...
/* 10. rho=(-[interaction_matrix*utilde]^{H}u]/(||interaction_matrix*utilde||^2)
      interaction_matrix*utilde */
         ...
         tempdc=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            tempdc=fcomplex_add(tempdc,fcomplex_mul(fcomplex_conj(zd[index0i]),zd[index0i]));
         }
         /* Check for iterative.breakdown */
...
/* rho=[interaction_matrix*utilde]^{H}u */
         rho=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            rho=fcomplex_add(rho,fcomplex_mul(fcomplex_conj(zd[index0i]),u[index0i]));
         }
...
/* 13. zd=u,zg=0,zomega=0 */
         ...
         memcpy(zomega,zg,(size_t)target.Nd3*sizeof(fcomplex));
...
/* tempf=q[ii]^{H}u */
         ...
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<target.Nd;index0i++){
               tempf=fcomplex_add(tempf,fcomplex_mul(fcomplex_conj(q[array1+index0i]),u[array0+index0i]));
            }
         }
...
/* 14. beta=(-q[s]^{H}zd)/(c[s-1]) */
               ...
               for(vc=0;vc<3;vc++){
                  ...
                  for(index0i=0;index0i<target.Nd;index0i++){
                     beta=fcomplex_add(beta,fcomplex_mul(fcomplex_conj(q[array1+index0i]),zd[array0+index0i]));
                  }
               }
               /* Check for iterative.breakdown */
...
/* 19. beta=(-q[0]^{H}zomega)(rho*c[iterative.vec-1]) */
         beta=zero;
         for(index0i=0;index0i<target.Nd3;index0i++){
            beta=fcomplex_add(beta,fcomplex_mul(fcomplex_conj(q[index0i]),zomega[index0i]));
         }
...
/* 22. beta=(-q[s]^{H}zomega)/(c[s-1]) */
            ...
            for(vc=0;vc<3;vc++){
               ...
               array1=s*target.Nd3+vc*target.Nd;
               for(index0i=0;index0i<target.Nd;index0i++){
                  beta=fcomplex_add(beta,fcomplex_mul(fcomplex_conj(q[array1+index0i]),zomega[array0+index0i]));
               }
            }
            /* Check for iterative.breakdown */
            if(fcomplex_abs(c[s-1])<iterative.breakdown){
...
/* 26. c[ii-1]=q[ii]^{H}d[ii-1] */
         ...
         for(vc=0;vc<3;vc++){
            ...
            array1=ii*target.Nd3+vc*target.Nd;
            for(index0i=0;index0i<target.Nd;index0i++){
               c[ii-1]=fcomplex_add(c[ii-1],fcomplex_mul(fcomplex_conj(q[array1+index0i]),d[array0+index0i]));
            }
         }

/* ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->-> */
/* 27. g[ii-1]=zg */
         memcpy(&g[(ii-1)*target.Nd3],zg,(size_t)target.Nd3*sizeof(fcomplex));
...
/* 40. g[iterative.vec-1]=zg */
      memcpy(&g[(iterative.vec-1)*target.Nd3],zg,(size_t)target.Nd3*sizeof(fcomplex));
