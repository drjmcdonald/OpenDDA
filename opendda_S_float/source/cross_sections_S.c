...
/* C_{ext} Extinction cross section
   ...
   cross_section.extinction=(4.0F*fpi*wavenumber*temp)/E2.dat[0];
...
/* C_{abs} Absorption cross section
   ...
   cross_section.absorption=0.0F;
   for(index0i=0;index0i<target.Nd3;index0i++){
      temp=fcomplex_abs2(dipole_polarisation[index0i]);
      cross_section.absorption+=(-interaction_matrix_diagonal[index0i].dat[1]*temp-(k3*temp));
   }
   cross_section.absorption*=((4.0F*fpi*wavenumber)/E2.dat[0]);
