...
/* 3. beta=sqrt[vtilde^{T}vtilde]=sqrt[r^{T}r],vtilde=r */
   beta=zero;
   for(index0i=0;index0i<target.Nd3;index0i++){
      beta=fcomplex_add(beta,fcomplex_mul(r[index0i],r[index0i]));
   }
...
/* interaction_matrix*v=M^{-1}interaction_matrix*v */
      ...
      for(vc=0;vc<3;vc++){
         ...
         array1=vc*target.Nv;
         for(index0i=0;index0i<target.Nd;index0i++){
            alpha=fcomplex_add(alpha,fcomplex_mul(v[array0+index0i],zero_padded_vector[array1+index0i]));
         }
      }
...
/* interaction_matrix*v is stored in zero_padded_vector */
         for(vc=0;vc<3;vc++){
            ...
            for(index0i=0;index0i<target.Nd;index0i++){
               array2=array0+index0i;
               vtilde[array2]=fcomplex_sub(zero_padded_vector[array1+index0i],fcomplex_add(fcomplex_mul(alpha,
                              v[array2]),fcomplex_mul(vtilde[array2],beta)));
            }
         }
      }
...
/* 12. beta=sqrt[vtilde^{T}vtilde],omega=||vtilde||/|beta| */
      beta=zero;
      for(index0i=0;index0i<target.Nd3;index0i++){
         beta=fcomplex_add(beta,fcomplex_mul(vtilde[index0i],vtilde[index0i]));
      }
      beta=fcomplex_sqrt(beta);
      /* Check for iterative.breakdown */
      if(fcomplex_abs(beta)<iterative.breakdown){
...
/* 17. p=(v-eta*p-theta*pold)/zeta (tempdc0=1/zeta) */
         for(index0i=0;index0i<target.Nd3;index0i++){
            ...
            pold[index0i]=fcomplex_mul(fcomplex_sub(v[index0i],
                           fcomplex_add(fcomplex_mul(eta,p[index0i]),fcomplex_mul(theta,pold[index0i]))),tempdc0);
         }
